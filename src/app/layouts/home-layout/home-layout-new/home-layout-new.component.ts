...
export class HomeLayoutNewComponent implements OnInit {
  ...
  quantityApprove: any = {};
  iconNotify = environment.host + 'api/share/software/icon/megaphone.png';
  ...
  ngOnInit(): void {
    this.appUserService.user$.subscribe(() => {
      ...
      this.isHR =
        ...
        this.departmentHRs.includes(this.departmentID) ||
        false;
  ...
  getMenus() {
    ...
    this.menuAppService.getAll().subscribe({
      next: (response) => {
        ...
        this.menus = this.menuAppService.sortBySTTImmutable(
          this.menus,
          (i) => i.STT ?? i.stt ?? 0,
        );
        ...
        this.menuWeekplans = pesons.Children.find(
          (x: any) => x.Code == 'planweek',
        );
  ...
  getNewsletterImageUrl(item: any): string {
    ...
    let urlImage = (serverPath || imageName || '').replace(
      '\\\\192.168.1.190\\',
      '',
    );
