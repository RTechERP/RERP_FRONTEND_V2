...
export class MainLayoutComponent implements OnInit {
  ...
  menus: MenuItem[] = [
    { kind: 'leaf', key: 'crm', title: 'Khách hàng', isOpen: true, icon: 'assets/icon/layers.png', comp: CustomerComponent },
    { kind: 'leaf', key: 'sales', title: 'Bán hàng', isOpen: true, icon: 'assets/icon/layers.png', comp: CustomerComponent },
    {
      kind: 'group', key: 'product', title: 'ProductRTC', isOpen: true, icon: 'assets/icon/layers.png',
      children: [
        { kind: 'leaf', key: 'product1', title: 'Product 1', isOpen: true, comp: TbProductRtcComponent, icon: 'assets/icon/layers.png' },
        { kind: 'leaf', key: 'product2', title: 'Product 2', isOpen: true, comp: TbProductRtcComponent /* không icon */ },
      ]
    },
    { kind: 'leaf', key: 'project', title: 'DỰ ÁN', isOpen: false, icon: null, comp: ProjectComponent },
  ];
  dynamicTabs: TabItem[] = [];
  ...
  newTab(comp: Type<any>, title: string, injector?: Injector) {
    ...
    if (idx >= 0) { this.selectedIndex = idx; return; }
  ...
  closeTab({ index }: { index: number }) {
    ...
    if (this.selectedIndex >= this.dynamicTabs.length) this.selectedIndex = this.dynamicTabs.length - 1;
  }
