...
export interface CourseKPIEmployeeTeamModel {
  ID: number;
  Name: string;
  DepartmentID: number;
  LeaderID: number;
...
@Component({
  selector: 'app-course-kpi-employee-team-detail',
  imports: [
    CommonModule,
    FormsModule,
    NzInputModule,
  ...
  templateUrl: './course-kpi-employee-team-detail.component.html',
  styleUrl: './course-kpi-employee-team-detail.component.css'
})
export class CourseKpiEmployeeTeamDetailComponent {
  @Input() id: number = 0;
  ...
  // Form model
  model: CourseKPIEmployeeTeamModel = {
    ID: 0,
    Name: '',
    DepartmentID: 0,
    LeaderID: 0,
  ...
  loadEmployees(): void {
    this.courseKpiEmployeeTeamService.getEmployees(0, 0).subscribe({
      next: (response) => {
        if (response?.status === 1) {
  ...
  save(): void {
    ...
    this.courseKpiEmployeeTeamService.saveData(saveData).subscribe({
      next: (response) => {
        if (response?.status === 1) {
