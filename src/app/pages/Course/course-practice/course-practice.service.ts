...
export class CoursePracticeService {
  private apiUrl = environment.host + 'api/coursepractice/';
  ...
  CourseLessonByCourseID(courseID: number): Observable<any> {
    ...
    return this.http.get<any>(this.apiUrl + `get-course-leson-by-course-id`, { params });
  }

  // Cập nhật trạng thái status leson history
  ChangeStatusLessonHistory(LessonID: number, completed: boolean): Observable<any> {
    return this.http.post<any>(this.apiUrl + 'change-status-lesson-history?lessonId=' + LessonID + '&completed=' + completed, {});
  }

  // Lấy kết quả bài thi
  GetResultExam(courseID: number, lessonID: number): Observable<any> {
    return this.http.post<any>(this.apiUrl + `get-exam-result?courseID=${courseID}&lessonID=${lessonID}`, {});
  }

  // Tạo bài thi
  CreateExamResult(courseExamID: number): Observable<any> {
    return this.http.post<any>(this.apiUrl + `create-exam-result?courseExamID=${courseExamID}`, {});
  }

  // Lấy danh sách câu hỏi 
  ListExamQuestion(courseId: number = 0, courseExamResultID: number = 0, examType: number = 1, lessonID: number = 0): Observable<any> {
    return this.http.get<any>(this.apiUrl + `list-exam-question?courseId=${courseId}&courseExamResultID=${courseExamResultID}&examType=${examType}&lessonID=${lessonID}`);
  }
  ...
  SubmitExamResult(courseExamResultID: number): Observable<any> {
    return this.http.post<any>(this.apiUrl + `submit-exam?courseExamResultID=${courseExamResultID}`, {});
  }

  // Lấy kết quả đúng của bài thi
  GetQuestionAnswerRight(courseExamResultId: number): Observable<any> {
    return this.http.get<any>(this.apiUrl + `get-question-answer-right?courseExamResultId=${courseExamResultId}`);
  }
  ...
  GetHistoryResultPractice(courseExamId: number): Observable<any> {
    return this.http.get<any>(this.apiUrl + `get-history-result-practice?courseExamId=${courseExamId}`);
  }

  // Lấy kết quả chi tiết của một lần thi thực hành
  GetResultPractice(courseResultId: number): Observable<any> {
    return this.http.get<any>(this.apiUrl + `get-result-practice?courseResultId=${courseResultId}`);
  }

  // Lấy danh sách câu hỏi thực hành
  GetPracticeQuestions(courseExamId: number): Observable<any> {
    return this.http.get<any>(this.apiUrl + `get-question?courseExamId=${courseExamId}`);
  }

  // Xác nhận hoàn thành bài thi thực hành
  ConfirmPractice(resultId: number): Observable<any> {
    return this.http.post<any>(this.apiUrl + `confirm-practice`, { ID: resultId });
  }
  ...
  GetQuestionDetails(questionId: number, courseId: number): Observable<any> {
    return this.http.get<any>(this.apiUrl + `question-details?questionId=${questionId}&courseId=${courseId}`);
  }
