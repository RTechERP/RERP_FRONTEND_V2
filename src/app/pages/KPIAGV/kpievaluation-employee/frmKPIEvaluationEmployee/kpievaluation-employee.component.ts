...
@Component({
  selector: 'app-kpiagv-evaluation-employee',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
...
export class KPIAGVEvaluationEmployeeComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  loadKPISession(): void {
    this.KPIAGVService.getDataKPISession(this.txtYear, this.departmentID, this.txtKeywords).subscribe({
      next: (res) => {
        if (res.data) {
  ...
  loadKPIExam(kpiSessionID: number): void {
    this.KPIAGVService.getDataKPIExam(this.employeeID, kpiSessionID).subscribe({
      next: (res) => {
        if (res.data) {
  ...
  loadPositionAndTeam(kpiSessionID: number): void {
    forkJoin({
      teamRes: this.KPIAGVService.getComboboxTeam(kpiSessionID).pipe(catchError(() => of({ data: [] }))),
      positionRes: this.KPIAGVService.getPositionEmployee(kpiSessionID).pipe(catchError(() => of({ data: [] })))
    }).subscribe({
      next: (results) => {
    ...
    if (args?.grid?.getSelectedRows().length > 0) {
      ...
      if (selectedRow) {
        if (this.selectedExamID !== selectedRow.ID) {
          ...
          this.KPIAGVService.getIsPublish(this.selectedExamID, this.isPublic, this.employeeID, this.selectedSessionID)
  ...
  btnChoicePosition_Click(): void {
    ...
    this.modal.confirm({
      ...
      nzOnOk: () => {
        ...
        this.KPIAGVService.choicePosition(positionID).subscribe({
          next: (res) => {
            if (res.status === 1) {
  ...
  btnEmployeeApproved_Click(): void {
    ...
    const modalRef = this.modalService.open(KPIAGVEvaluationFactorScoringDetailsComponent, {
      centered: true,
      backdrop: 'static',
      keyboard: false,
  ...
  btnSuccessKPI_Click(): void {
    ...
    this.KPIAGVService.checkComplete(this.selectedExamID).subscribe({
      next: (res) => {
        ...
        if (res.data && res.data.length > 0) {
          ...
          this.modal.confirm({
            ...
            nzCancelText: 'Hủy',
            nzOnOk: () => {
              this.KPIAGVService.confirmSuccessKPI(this.selectedExamID).subscribe({
                next: (confirmRes) => {
  ...
  loadDataDetails(): void {
    ...
    this.KPIAGVService.loadKPIKyNang(this.selectedExamID, this.isPublic, this.employeeID)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (res) => {
          if (res.data) {
  ...
  private loadRemainingTabsInBackground(): void {
    ...
    const tabChung$ = this.KPIAGVService.loadKPIChung(this.selectedExamID, this.isPublic, this.employeeID);
    const tabChuyenMon$ = this.KPIAGVService.loadKPIChuyenMon(this.selectedExamID, this.isPublic, this.employeeID);
    const tabRuleTeam$ = this.KPIAGVService.loadKPIRuleAndTeam(this.selectedExamID, this.isPublic, this.employeeID, this.selectedSessionID);
    ...
    forkJoin({
      ...
      next: (results) => {
        ...
        if (results.ruleTeam?.data) {
          ...
          this.KPIAGVService.getFinalPoint(this.employeeID, this.selectedSessionID).subscribe({
            next: (finalRes: any) => {
              if (finalRes.data) {
  ...
  private loadPointRuleNewAndCalculate(): void {
    this.KPIAGVService.loadPointRuleNew2(this.selectedExamID, this.isPublic, this.employeeID, this.selectedSessionID).subscribe({
      next: (res: any) => {
        // Bước 2: Merge kết quả API vào dataRule
  ...
  private loadPointRuleLastMonthNewAndCalculate(): void {
    this.KPIAGVService.loadPointRuleLastMonthNew(this.selectedExamID, this.isPublic, this.employeeID, this.selectedSessionID).subscribe({
      next: (res: any) => {
        // Bước 2: Merge kết quả API vào dataRule
        if (res?.data && Array.isArray(res.data)) {
          const lstResult: any[] = res.data;
