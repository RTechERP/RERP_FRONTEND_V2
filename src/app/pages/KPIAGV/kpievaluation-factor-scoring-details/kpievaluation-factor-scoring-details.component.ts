...
import { KPIAGVService } from '../kpi-service/kpi.service';
import { KPIAGVEvaluationFactorScoringDetailsService } from './kpievaluation-factor-scoring-details-service/kpievaluation-factor-scoring-details.service';
import { ReadOnlyLongTextEditor } from '../kpievaluation-employee/frmKPIEvaluationEmployee/readonly-long-text-editor';
import { KPIAGVCriteriaViewComponent } from '../kpicriteria-view/kpicriteria-view.component';
import { KPIAGVRuleSumarizeTeamChooseEmployeeComponent } from '../kpi-rule-sumarize-team-choose-employee/kpi-rule-sumarize-team-choose-employee.component';
...
@Component({
  selector: 'app-kpiagv-evaluation-factor-scoring-details',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
...
export class KPIAGVEvaluationFactorScoringDetailsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private KPIAGVService = inject(KPIAGVEvaluationFactorScoringDetailsService);
  private kpiSharedService = inject(KPIAGVService);
  ...
  ngOnInit(): void {
    ...
    this.KPIAGVService.getTeam().subscribe(res => {
      this.team = res;
      this.applyVisibilityRules();
  ...
  private initRuleGrid(): void {
    ...
    const monthColumnFormatter = (row: number, cell: number, value: any, columnDef: any, dataContext: any) => {
      ...
      let bgColor = '';
      if (dataContext.__hasChildren || isNewLine) bgColor = '#D3D3D3';
  ...
  private loadComboboxData(): void {
    ...
    this.KPIAGVService.getComboboxSession().subscribe({
      next: (sessions) => {
  ...
  private loadKPIExams(kpiSessionId: number): void {
    ...
    this.KPIAGVService.getComboboxExam(kpiSessionId).subscribe({
      next: (exams) => {
        this.kpiExams = exams;
  ...
  private loadEmployees(): void {
    this.KPIAGVService.getComboboxEmployee().subscribe({
      next: (employees) => {
  ...
  private loadPointRuleNewAndCalculateDetail(): void {
    ...
    this.KPIAGVService.loadPointRuleNewDetail(examID, true, empID, sessionID)
    ...
    this.kpiSharedService.getAllTeamByEmployeeID(empID, kpiSessionID)
      ...
      .subscribe({
        next: (response: any) => {
          ...
          const modalRef = this.ngbModal.open(KPIAGVRuleSumarizeTeamChooseEmployeeComponent, {
            size: 'lg',
            backdrop: 'static'
          });
  ...
  btnUpdateDataRow_Click(): void {
    ...
    this.KPIAGVService.updateRowRule(kpiExamID, isAmdinConfirm, employeeID, kpiSessionID).subscribe({
      next: (response: any) => {
        ...
        const fiveS = this.formatDecimalNumber(this.getGridSummary(this.angularGridTeam, 'FiveS') || 0, 2);
        const reportWork = this.formatDecimalNumber(this.getGridSummary(this.angularGridTeam, 'ReportWork') || 0, 2);
        const customerComplaint = this.formatDecimalNumber(this.getGridSummary(this.angularGridTeam, 'CustomerComplaint') || 0, 2);
        const deadlineDelay = this.formatDecimalNumber(this.getGridSummary(this.angularGridTeam, 'DeadlineDelay') || 0, 2);
        const teamKPIKyNang = this.formatDecimalNumber(this.getGridSummary(this.angularGridTeam, 'KPIKyNang') || 0, 2);
        const teanKPIChung = this.formatDecimalNumber(this.getGridSummary(this.angularGridTeam, 'KPIChung') || 0, 2);
        const teamKPIPLC = this.formatDecimalNumber(this.getGridSummary(this.angularGridTeam, 'KPIPLC') || 0, 2);
    ...
    (shouldSaveRule ? this.saveRuleData() : of(true))
      .pipe(
        switchMap((isRuleSaved) => {
          ...
          return this.KPIAGVService.saveDataKPI(request).pipe(
            catchError((err) => {
  ...
  openCriteriaView(): void {
    ...
    try {
      const modalRef = this.ngbModal.open(KPIAGVCriteriaViewComponent, {
        size: 'xl',
        backdrop: 'static',
        keyboard: false,
