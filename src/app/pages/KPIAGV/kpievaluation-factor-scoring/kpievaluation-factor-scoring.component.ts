...
import { KPIAGVEvaluationFactorScoringDetailsComponent } from '../kpievaluation-factor-scoring-details/kpievaluation-factor-scoring-details.component';

@Component({
  selector: 'app-kpiagv-evaluation-factor-scoring',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
...
export class KPIAGVEvaluationFactorScoringComponent implements OnInit, AfterViewInit {
  // Grid instances
  ...
  // Inject services
  private KPIAGVService = inject(KPIAGVService);
  ...
  initEmployeeGrid(): void {
    ...
    this.employeeGridOptions = {
      enableAutoResize: true,
      autoResize: {
        container: '#' + this.gridIdPrefix + '-employee-container',
        calculateAvailableSizeBy: 'container',
        resizeDetection: 'container'
      },
      gridWidth: '100%',
  ...
  loadDataDetails(): void {
    ...
    this.KPIAGVService.loadKPIKyNangFactorScoring(this.selectedExamID, this.isPublic, this.selectedEmployeeID)
  ...
  private loadRemainingTabsInBackground(): void {
    ...
    this.loadingAGVTabs = true;
    const sessionID = this.selectedKPISessionID || 0;

    // Tạo các observable cho mỗi tab
    // Tab Chung (Evaluation4 in UI / Tab 2 logic)
    const tabChung$ = this.KPIAGVService.loadKPIChungFactorScoring(this.selectedExamID, this.isPublic, this.selectedEmployeeID);
    // Tab Chuyên môn (Evaluation2 in UI / Tab 3 logic)
    const tabChuyenMon$ = this.KPIAGVService.loadKPIChuyenMonFactorScoring(this.selectedExamID, this.isPublic, this.selectedEmployeeID);
    ...
    const tabRuleTeam$ = this.KPIAGVService.loadKPIRuleAndTeamFactorScoring(this.selectedExamID, this.isPublic, this.selectedEmployeeID, sessionID);
  ...
  loadDepartments(): void {
    this.KPIAGVService.getDepartments().subscribe({
      next: (res) => {
        if (res.data) {
  ...
  loadKPISession(): void {
    ...
    this.KPIAGVService.getComboboxKPISession(currentYear, this.selectedDepartmentID).subscribe({
      next: (res) => {
        if (res.data) {
  ...
  loadUserTeam(): void {
    ...
    this.KPIAGVService.getComboboxTeamKPI(this.selectedKPISessionID, this.selectedDepartmentID).subscribe({
      next: (res) => {
        if (res.data) {
  ...
  loadKPIExam(): void {
    ...
    this.KPIAGVService.getKpiExamByKsID(this.selectedKPISessionID, this.selectedDepartmentID).subscribe({
      next: (res: any) => {
        if (res.data) {
  ...
  loadEmployee(): void {
    ...
    this.KPIAGVService.getListEmployeeKPIEvaluation(
      this.selectedExamID,
      this.selectedStatus,
      ...
      next: (res: any) => {
        ...
        if (res.data) {
          ...
          this.dataEmployee = res.data.map((item: any, index: number) => ({
            ...item,
            // Sử dụng index làm id để đảm bảo tính duy nhất tuyệt đối cho SlickGrid DataView
            // Tránh việc trùng lặp nếu ID database trùng với giá trị index
            id: index
          }));
  ...
  btnEmployeeApproved_Click(): void {
    ...
    const modalRef = this.ngbModal.open(KPIAGVEvaluationFactorScoringDetailsComponent, {
      centered: true,
      backdrop: 'static',
      keyboard: false,
  ...
  updateStatusKPI(status: number): void {
    ...
    this.KPIAGVService.checkUpdateStatusKPI(status, this.selectedExamID, this.selectedEmployeeID).subscribe({
      next: (res) => {
        if (res.status === 1) {
          ...
          this.modal.confirm({
            ...
            nzOnOk: () => {
              ...
              this.KPIAGVService.updateStatusKPIAction(status, this.selectedExamID, this.selectedEmployeeID).subscribe({
                next: (updateRes) => {
  ...
  btnLoadDataTeam_Click(): void {
    ...
    this.KPIAGVService.getAllTeamByEmployeeID(empID, kpiSessionID)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (response: any) => {
          ...
          modalRef.closed.subscribe({
            next: (lstEmpChose: any[]) => {
              ...
              this.KPIAGVService.loadDataTeam(loadRequest)
                .pipe(takeUntil(this.destroy$))
                .subscribe({
                  next: (loadResponse: any) => {
                    ...
                    const sessionID = this.selectedKPISessionID || 0;
                    this.KPIAGVService.loadPointRuleNew(this.selectedExamID, this.selectedEmployeeID, sessionID)
                      .pipe(takeUntil(this.destroy$))
                      .subscribe({
  ...
  btnExportExcelByTeam_Click(): void {
    ...
    this.KPIAGVService.exportExcelByTeam(this.selectedKPISessionID, this.selectedDepartmentID)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (blob: Blob) => {
 ...
 */
  ...
  btnAdminConfirm_Click(): void {
    ...
    this.KPIAGVService.checkUpdateStatusKPI(0, kpiExamID, empID).subscribe({
      next: (response: any) => {
        if (response.status === 1) {
          ...
          this.modal.confirm({
            ...
            nzOnOk: () => {
              ...
              this.KPIAGVService.adminConfirmKPI(kpiExamID, empID).subscribe({
                next: (confirmRes: any) => {
  ...
  private saveDataRule(): void {
    try {
      ...
      // Call API
      this.KPIAGVService.saveDataRule(request).subscribe({
        next: (response: any) => {
          if (response.status === 1) {
  ...
  private loadPointRuleNewAndCalculate(): void {
    const sessionID = this.selectedKPISessionID || 0;
    // Note: isTBP used inside applyTeamSummaryAndCalculate

    this.KPIAGVService.loadPointRuleNew(this.selectedExamID, this.selectedEmployeeID, sessionID)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (res) => {
          // lstResult = kết quả từ SP spGetSumarizebyKPIEmpPointIDNew (tương đương WinForms)
          const lstResult: any[] = (res.data && Array.isArray(res.data)) ? res.data : [];
          const selectedEmployee = this.dataEmployee.find(e => (e.EmployeeID || e.ID || e.id) === this.selectedEmployeeID);
          const isAdminConfirm = selectedEmployee?.IsAdminConfirm || false;
          const pointID = selectedEmployee?.ID || 0;
          const hasDetails = pointID > 0 && this.dataRule.some((r: any) =>
            (typeof r.EmpPointDetailID === 'number' && r.EmpPointDetailID > 0) ||
            (r.KPIEmployeePointID === pointID) ||
            r.EmpPointDetailID > 0
          );

          if (lstResult.length === 0 && !isAdminConfirm) {
            this.applyTeamSummaryAndCalculate(false, [], isAdminConfirm);
            return;
          }

          if (isAdminConfirm && hasDetails) {
            // Nếu admin đã duyệt và đã có điểm detail, không load đè từ SP summary
            console.log('[FactorScoring] Admin confirmed. Skipping overwrite.');
          ...
          if (lstResult.length > 0) {
            ...
            this.dataRule = this.dataRule.map(rule => {
              const ruleCode = rule.EvaluationCode?.toUpperCase().trim();
              const newData = lstResult.find((item: any) => item.EvaluationCode?.toUpperCase().trim() === ruleCode);

              if (newData) {
                if (!hasDetails) {
                  // Mới hoàn toàn: Cập nhật cả 3 tháng (Tương đương C# LoadPointRuleNew)
                  return {
                    ...rule,
                  ...
                  // Đã có dữ liệu nhưng chưa confirm: Cập nhật tháng 3 (Tương đương C# LoadPointRuleLastMonthNew)
                  return {
                    ...rule,
                    ThirdMonth: newData.ThirdMonth
                  };
          ...
          // Sau khi merge dữ liệu từ API, thực hiện tính toán từ Team Rule
          this.calculatorNoError();
          this.applyTeamSummaryAndCalculate(hasDetails, lstResult, isAdminConfirm);
        },
        error: (err) => {
          ...
          this.calculatorNoError();
          this.applyTeamSummaryAndCalculate(false, [], false);
        }
      });
  }
  ...
  private applyTeamSummaryAndCalculate(hasDetails: boolean = false, lstResult: any[] = [], isAdminConfirm: boolean = false): void {
    ...
    this.KPIAGVService.getFinalPoint(this.selectedEmployeeID, this.selectedKPISessionID || 0).subscribe({
      next: (finalRes: any) => {
        if (finalRes.data) {
  ...
  private calculatorNoError(): void {
    ...
    const filteredNodes = this.dataRule.filter((row: any) => {
      ...
      return this.listCodesNoError.some(c => c.toUpperCase().trim() === code);
    });
