...
export class ScanBillComponent implements OnInit, AfterViewInit {
  ...
  loadBillExportQR() {
    ...
    this.billExportService.getBillExportQR(1, this.searchParams.keyword).subscribe({
      next: (res) => {
        if (res?.data && Array.isArray(res.data)) {
          const newRows = res.data;

          if (this.table_BillExport) {
            const existingIDs = this.table_BillExport.getData().map((row: any) => row.ID);
            ...
            this.dataTableBillExport = [...this.dataTableBillExport, ...newRows];
          }
        }
      },
      error: (err) => {
  ...
  async IsApprovedAll(apr: boolean) {
    ...
    for (const row of allRows) {
      try {
        ...
        const fullResponse = await firstValueFrom(this.billExportService.getBillExportByID(row.ID));
        ...
        // Gọi API duyệt phiếu
        await firstValueFrom(this.billExportService.approved(fullBillExport, apr));
  
        // ✅ Gọi lại API lấy lại dữ liệu mới nhất của phiếu sau khi duyệt
        const updatedRes = await firstValueFrom(this.billExportService.getBillExportByID(row.ID));
    ...
    if (this.table_BillExport) {
      ...
      this.table_BillExport = new Tabulator('#table_billExportQR', {
        ...
        columns: [
          ...
          {
            title: 'Nhận chứng từ', field: 'IsApproved', formatter: (cell) => {
              const value = cell.getValue();
          ...
          { title: 'Trạng Thái', field: 'nameStatus', hozAlign: 'left', headerHozAlign: 'center' },
          {
          ...
          {
            title: 'Chuẩn bị xong', field: 'IsPrepared', formatter: (cell) => {
              const value = cell.getValue();
              return `<input type="checkbox" ${value === true ? 'checked' : ''} disabled />`;
            }, hozAlign: 'center', headerHozAlign: 'center'
          },
          {
            title: 'Đã nhận hàng', field: 'IsReceived', formatter: (cell) => {
              const value = cell.getValue();
              return `<input type="checkbox" ${value === true ? 'checked' : ''} disabled />`;
            }, hozAlign: 'center', headerHozAlign: 'center'
          },
          { title: 'Số phiếu', field: 'Code', hozAlign: 'left', headerHozAlign: 'center' },
          { title: 'Phòng ban', field: 'DepartmentName', hozAlign: 'left', headerHozAlign: 'center' },
          { title: 'Mã NV', field: 'EmployeeCode', hozAlign: 'left', headerHozAlign: 'center' },
          { title: 'Tên NV', field: 'FullName', hozAlign: 'left', headerHozAlign: 'center' },
          { title: 'Khách hàng', field: 'CustomerName', hozAlign: 'left', headerHozAlign: 'center' },
          { title: 'Nhà cung cấp', field: 'NameNCC', hozAlign: 'left', headerHozAlign: 'center' },
          ...
          { title: 'Loại vật tư', field: 'WarehouseType', hozAlign: 'left', headerHozAlign: 'center' },
          { title: 'Kho', field: 'WarehouseName', hozAlign: 'left', headerHozAlign: 'center' },
          { title: 'Loại phiếu', field: 'ProductTypeText', hozAlign: 'left', headerHozAlign: 'center' },
          { title: 'Người giao', field: 'FullNameSender', hozAlign: 'left', headerHozAlign: 'center' },
        ],
      });
