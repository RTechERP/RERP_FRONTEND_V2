import { Component, OnInit, AfterViewInit, ViewChild, Input, Type, ApplicationRef, EnvironmentInjector, createComponent, Output, EventEmitter } from '@angular/core';
...
export class BillDocumentImportComponent implements OnInit, AfterViewInit {
  ...
  getBillDocumentImportLog(bdeID: number, documentImportID:number) {
    this.billImportService.getBillDocumentImportLog(bdeID, documentImportID).subscribe({
        next: (res) => {
            if (res?.data && Array.isArray(res.data)) {
                this.dataBillDocumentImportLog = res.data;
                this.table_billDocumentImportLog?.replaceData(this.dataBillDocumentImportLog);
            }
        },
        error: (err) => {
  ...
  saveData() {
    ...
    for (const item of currentData) {
      if (item._edited) {
        // Validate
        if (!item.DocumentStatus || isNaN(item.DocumentStatus) || item.DocumentStatus <= 0) {
          this.notification.error('Lỗi', `Vui lòng nhập Trạng thái cho chứng từ [${item.DocumentImportCode || 'N/A'}].`);
          return;
        }
  
        if (item.DocumentStatus === 2 && (!item.ReasonCancel || item.ReasonCancel.trim() === '')) {
          this.notification.error('Lỗi', `Vui lòng nhập Lý do hủy cho chứng từ [${item.DocumentImportCode || 'N/A'}].`);
          return;
        }
    ...
    this.billImportService.saveBillDocumentImport(changedData).subscribe({
      next: (res) => {
        if (res.status === 1) {
          ...
          this.notification.error('Lỗi', res.error || 'Có lỗi xảy ra khi lưu dữ liệu.');
        }
      },
      error: (err) => {
  ...
  drawTable() {
    ...
    if (!this.table_billDocumentImport) {
      this.table_billDocumentImport = new Tabulator("#table_billDocumentImport", {
        index: "ID",
        ...
        columns: [
            ...
            formatter: (cell) => {
              ...
              if (!val && val !== 0 ||val ==0) return '<div class="d-flex justify-content-between align-items-center"><p class="w-100 m-0 text-muted"></p><i class="fas fa-angle-down"></i></div>';
              const st = this.cbbStatus.find((p: any) => p.ID === parseInt(val) || p.ID === val);
          ...
          },
          {
            ...
            formatter: (cell) => {
              const val = cell.getValue();      
              if (!val && val !== 0 ||val ==0) return '<div class="d-flex justify-content-between align-items-center"><p class="w-100 m-0 text-muted"></p><i class="fas fa-angle-down"></i></div>';
              const st = this.cbbStatusPur.find((p: any) => p.ID === parseInt(val) || p.ID === val);
              return `<div class="d-flex justify-content-between align-items-center"><p class="w-100 m-0">${st ? st.Name : val}</p><i class="fas fa-angle-down"></i></div>`;
            },
          },
          {
          ...
          { title: "Mã chứng từ", field: "DocumentImportCode", hozAlign: "left", headerHozAlign: "center" },
          { title: "Tên chứng từ", field: "DocumentImportName", hozAlign: "left", headerHozAlign: "center" },
          { title: "Lý do", field: "ReasonCancel", hozAlign: "left", headerHozAlign: "center", editor: 'input', width: 200 },
          { title: "Ghi chú", field: "Note", hozAlign: "left", headerHozAlign: "center", editor: 'input', width: 200 },
          { title: "Người thay đổi", field: "UpdatedBy", hozAlign: "left", headerHozAlign: "center" },
    ...
    if (!this.table_billDocumentImportLog) {
      this.table_billDocumentImportLog = new Tabulator("#table_billDocumentImportLog", {
        index: "ID",
        ...
        selectableRows: 1,
        columns: [
          { title: "Trạng thái", field: "DocumentStatusText", hozAlign: "left", headerHozAlign: "center" },
          { title: "Mã chứng từ", field: "DocumentImportCode", hozAlign: "left", headerHozAlign: "center" },
          { title: "Tên chứng từ", field: "DocumentImportName", hozAlign: "left", headerHozAlign: "center" },
