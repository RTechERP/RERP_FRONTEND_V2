...
export class HistoryBorrowSaleComponent implements OnInit, AfterViewInit {
  ...
  loadData() {
    ...
    this.historyBorrowSaleService.getHistoryBorrowSale(
      ...
      next: (res: any) => {
        ...
        if (this.table) {
          ...
          console.log("datatesst", this.dataTable);
        } else {
          // Trường hợp này sẽ không xảy ra nếu loadData() được gọi sau drawTable()
          console.error('Lỗi: Bảng Tabulator chưa được khởi tạo khi loadData() được gọi.');
        }
      },
      error: (err: any) => {
  ...
  IsApprovedReturned(apr: boolean) {
    ...
    if (hasInvalidId) {
      this.notification.error("Thông báo", "Dữ liệu không hợp lệ: Một số phiếu không có ID!");
      return;
    }
