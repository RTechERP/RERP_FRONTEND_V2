...
export class ReportImportExportComponent implements OnInit, AfterViewInit {
    ...
    this.productsaleService.getDataProductSalebyID(this.productID).subscribe({
      next: (res) => {
        if (res?.data) {
          ...
          this.productsaleService.getDataLocation(this.newProductSale.ProductGroupID).subscribe({
            next: (locationRes) => {
              if (locationRes?.data) {
          ...
          this.notification.warning('Thông báo', res.message || 'Không thể lấy thông tin sản phẩm!');
        }
      },
      error: (err) => {
  ...
  getReport() {
    ...
    this.reportImportExportService.getReportImportExport(
      dateStart, dateEnd,
  ...
  async exportExcel() {
    ...
    this.tableReport = new Tabulator('#table_productsale', {
      ...
      columns: [
        ...
        { title: 'Mã sản phẩm', field: 'ProductNewCode', hozAlign: 'left', headerHozAlign: 'center' },
        ...
        { title: 'Tồn DK', field: 'TonDauKy', hozAlign: 'right', headerHozAlign: 'center', formatter: 'money', formatterParams: {precision: 2} },
        { title: 'Nhập', field: 'Import1', hozAlign: 'right', headerHozAlign: 'center', formatter: 'money', formatterParams: {precision: 2} },
        { title: 'Xuất', field: 'Export1', hozAlign: 'right', headerHozAlign: 'center', formatter: 'money', formatterParams: {precision: 2} },
        { title: 'Tồn CK', field: 'TonCuoiKy', hozAlign: 'right', headerHozAlign: 'center', formatter: 'money', formatterParams: {precision: 2} },
