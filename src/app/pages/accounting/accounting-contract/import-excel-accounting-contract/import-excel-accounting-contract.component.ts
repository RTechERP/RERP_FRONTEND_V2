...
@Component({
  selector: 'app-import-excel-accounting-contract',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './import-excel-accounting-contract.component.html',
  styleUrl: './import-excel-accounting-contract.component.css'
})
export class ImportExcelAccountingContractComponent implements OnInit {
  sheetNames: string[] = [];
  ...
  importData() {
    ...
    this.isLoading = true;
    this.AccountingContractService.importExcel(mappedData).subscribe({
      next: (res: any) => {
  ...
  downloadTemplate() {
    const filePath = '\\\\192.168.1.190\\Software\\Template\\ImportExcel\\Template_AccountingContract.xlsx';

    // Hiển thị loading message
    const msgId = this.notification.info('Thông báo', 'Đang tải file mẫu...', { nzDuration: 0 }).messageId;

    this.AccountingContractService.downloadFile(filePath).subscribe({
      next: (blob: Blob) => {
        ...
        if (blob && blob.size > 0) {
          ...
          link.href = url;
          link.download = 'Template_AccountingContract.xlsx';
