...
export class ExamListComponent implements OnInit, OnChanges, AfterViewInit, OnDestroy {
  ...
  private drawTable(): void {
    ...
    this.table = new Tabulator(this.tableRef.nativeElement, {
      ...
      groupBy: [
        (data: CourseData) => data.CourseCatalogTypeName || 'Chưa có loại khóa học',
        (data: CourseData) => data.CatalogName || 'Chưa có danh mục',
      ],
      groupStartOpen: [true, false], // Mở nhóm cấp 1, đóng nhóm cấp 2
      groupHeader: [
      ...
      //     return `<strong>Loại khóa học: ${value || 'Chưa phân loại'}</strong> (${count} khoá học)`;
      //   },
      // ],
      columns: [
        {
