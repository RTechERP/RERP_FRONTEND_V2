...
export class QuestionFormComponent implements OnInit, OnChanges {
  ...
  private loadQuestionData(): void {
    ...
    if (this.imagePath) {
      ...
      this.imagePreviewUrl = this.imagePath;
    } else {
      this.attachImageName = '';
      this.imagePreviewUrl = null;
    }
  ...
  removeImage(): void {
    ...
    getPathServer(): void {
    let serverPath = '';
    this.courseExamService.getPathServer('CourseExamExerciseImages').subscribe(res => {
      serverPath = (res?.data ?? '') + '/';
      const host = environment.host + 'api/share/';
    serverPath = (serverPath || '').replace("\\\\192.168.1.190\\", "");
    this.imagePreviewUrl = host + serverPath.replace(/\\/g, '/') + this.imagePath ;
    });
  ...
  getImageUrl(): string {
    ...
    //   serverPath = (res?.data ?? '') + '/';
    //   const host = environment.host + 'api/share/';
    // this.imagePath = (serverPath || '').replace("\\\\192.168.1.190\\", "");
    // this.imagePath = host +this.imagePath.replace(/\\/g, '/');
      ...
      this.courseExamService.uploadQuestionImage(this.selectedImageFile).subscribe(
        (res: any) => {
          if (res && res.status === 1 && res.data && Array.isArray(res.data) && res.data.length > 0) {
            ...
            resolve(fileInfo.SavedFileName || '');
          } else {
