...
import { CopyCourseCatalogModalComponent } from './copy-course-catalog-modal/copy-course-catalog-modal.component';
...
export class CourseWebManagementComponent implements OnInit, AfterViewInit {
...
lessonMenuBars : MenuItem[] = [];
  ...
  openCopyCatalogModal(): void {
    const modalRef = this.modalService.open(CopyCourseCatalogModalComponent, {
      centered: true,
      size: 'lg',
      backdrop: 'static',
      keyboard: false,
  ...
  draw_categoryTable(): void {
    ...
    if (this.categoryTable) {
      ...
      this.categoryTable = new Tabulator(this.categoryTableRef.nativeElement, {
        ...
        reactiveData: false,
        groupBy: 'CourseCatalogTypeName',
        groupStartOpen: true,
        groupHeader: [
          (value: any, count: number, data: any) => {
            return `<strong>Loại: ${value || 'Chưa phân loại'}</strong> (${count} danh mục)`;
          },
        ],
        columns: [
          {
