...
export class LessonDetailComponent implements OnInit, AfterViewInit {
  ...
  async saveLesson(): Promise<void> {
    ...
    // Validate VideoUrl if provided: must be a YouTube link
    const videoUrl = (this.formGroup.get('VideoUrl')?.value || '').toString().trim();
    if (videoUrl && !this.validateYoutubeUrl(videoUrl)) {
      this.notification.error(NOTIFICATION_TITLE.error, 'URL video không hợp lệ. Vui lòng nhập URL YouTube hợp lệ.');
      return;
    }
