...
export class SummaryOfExamResultsWebComponent implements OnInit, AfterViewInit {
  ...
  getEmployees() {
    this.examResultsService.getEmployees().subscribe({
      next: (response: any) => {
  ...
  getEmployeeData() {
    ...
    this.examResultsService.getEmployees(employeeID).subscribe({
      next: (response: any) => {
  ...
  getExamResultsByEmployeeID(employeeID: number) {
    ...
    this.examResultsService.getCourseSummary(employeeIDParam).subscribe({
      next: (response: any) => {
        if (response && response.status === 1 && response.data) {
