...
export class EmployeeAttendanceComponent implements OnInit, AfterViewInit {
  ...
  loadEmployees(): void {
    ...
    this.eas.getEmployee(0).subscribe({
      next: (res: any) => {
  ...
  private timeDisplay(dateVal: any, timeVal: any): string {
    ...
    // Bất khả kháng: chuỗi không đúng định dạng → để nguyên
    return s;
  }
  ...
  onDepartmentChange(): void {
    // lọc lại ds nhân viên theo phòng ban
    const filtered =
      this.departmentId && this.departmentId > 0
    ...
    // reset chọn nhân viên và tìm kiếm luôn
    this.employeeId = 0;
