...
import { PaymentOrder } from '../payment-order/model/payment-order';
...
@Component({
    selector: 'app-payment-order-employee',
    templateUrl: './payment-order-employee.component.html',
    styleUrls: ['./payment-order-employee.component.css'],
    standalone: true,
    imports: [
        CommonModule,
        FormsModule,
...
export class PaymentOrderEmployeeComponent implements OnInit {
    ...
    paymentOrderTypeOptions: any[] = [];
    headerGroups: any[][] = [];
    headerGroupsSpecial: any[][] = [];

    columnDefs: ColumnDef[] = [];
    columnDefsSpecial: ColumnDef[] = [];
    dataset: any[] = [];
    datasetSpecial: any[] = [];

    columnDefDetails: ColumnDef[] = [];
    columnDefDetailsSpecial: ColumnDef[] = [];
    datasetDetails: any[] = [];
    ...
    initColumns() {
        ...
        this.columnDefs = [
            ...
            { field: 'StartLocation', header: 'Điểm đi', width: '130px' },
            { field: 'EndLocation', header: 'Điểm đến', width: '130px' },
            { field: 'StatusBankSlip', header: 'TT Bank Slip', width: '120px' },
            // --- Lịch sử & Ghi chú ---
            { field: 'ContentLog', header: 'LS duyệt/hủy duyệt', width: '160px' },
            ...
            {
                field: 'ReasonRequestAppendFileHR',
                header: 'Lý do HR Y/c bổ sung',
                width: '160px',
            },
            // --- NV đăng ký ---
        ...
        this.headerGroupsSpecial = [
            [
                { header: 'Thông tin chung', colspan: 15 },
                { header: 'Lịch sử & Ghi chú', colspan: 3 },
            ],
        ];

        this.columnDefDetails = [
            { field: 'Stt', header: 'STT', width: '60px', cssClass: 'text-center' },
            {
        ...
        this.columnDefsSpecial = [
            ...
            {
                field: 'ContentLog',
                header: 'Lịch sử duyệt / hủy duyệt',
                width: '200px',
            },
            { field: 'ReasonCancel', header: 'Lý do hủy duyệt', width: '200px' },
            { field: 'Note', header: 'Ghi chú / Chứng từ kèm theo', width: '400px' },
        ];

        this.columnDefDetailsSpecial = [
            { field: 'Stt', header: 'STT', width: '60px', cssClass: 'text-center' },
            {
                field: 'ContentPayment',
        ...
        this.contextMenu = [
            {
                ...
                icon: 'pi pi-eye',
                command: (event) => {
                    let item = event.item ? (event.item as any).data : null;
                    // console.log('viewContract:', args);
                    let pathFolder = item?.FolderPath;
                ...
                label: 'Lịch sử duyệt/không duyệt',
                icon: 'pi pi-history',
                command: (event) => {
                    this.onOpenPaymentOrderLog(event.item ? (event.item as any).data : null);
                },
            }
        ]
        this.fileContextItems = [
            {
                label: 'Xem file',
                icon: 'pi pi-eye',
                command: (event) => {
                    this.onViewFile(event.item ? (event.item as any).data : null);
                },
            },
            {
                label: 'Tải file',
                icon: 'pi pi-download',
                command: (event) => {
                    this.onDownloadFile(event.item ? (event.item as any).data : null);
                },
            },
        ];
    }
    ...
    onCellActionFile(event: { field: string; rowData: any }) {
        if (event.field === 'FileName') {
            this.onViewFile(event.rowData);
        }
    }

    onRowSelectFile(event: any) {
        this.selectedRowFile = event;
    }
    ...
    loadDataCombo() {
        ...
        forkJoin({
            paymentTypes: this.paymentOrderTypeService.getAll(),
            steps: this.paymentService.getStep(),
        }).subscribe({
            next: (res: any) => {
                // 1. Payment Types
                this.paymentOrderTypes = res.paymentTypes.data;
                this.paymentOrderTypeOptions = (res.paymentTypes.data || []).map(
                    (t: any) => ({ label: t.TypeName, value: t.ID }),
                );

                // 2. Steps
                if (res.steps?.data) {
                    this.allSteps = res.steps.data;
                    this.filterSteps();
                }
                this.loadData();
    ...
    filterSteps() {
        ...
        const stepsData = this.allSteps.filter(
            (x: any) => x.FollowType === followType && x.Step !== 1,
        );
        this.steps = stepsData.map((x: any) => ({
            value: x.Step,
    ...
    loadData() {
        if (this.activeTab == '0') this.loadDataNormal();
        else this.loadDataSpecial();
    }
    ...
    loadDataNormal() {
        ...
        this.paymentService.get(p).subscribe({
            next: (response: any) => {
                this.dataset = (response.data || []).map((x: any) => ({
                    ...x,
                    id: x.ID,
                }));
    ...
    loadDataSpecial() {
        ...
        if (this.isPermisstionDB && this.isApprove) {
            ...
            emp = this.appUserService.currentUser?.EmployeeID || 0;
        }
    ...
    onEdit() {
        if (this.selectedItem) {
            this.initModal(this.selectedItem);
        } else {
            this.notification.warning(
                NOTIFICATION_TITLE.warning,
                'Vui lòng chọn bản ghi để sửa',
            );
        }
    }
    ...
    onDelete() {
        if (this.selectedItem) {
            Swal.fire({
                title: 'Xác nhận xóa?',
                text: `Bạn có chắc muốn xóa bản ghi [${this.selectedItem.Code}] không?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
    ...
    tabValueChange(event: any) {
        ...
        this.selectedItem = null;
        this.isLoading = true; // Hiển thị loading ngay lập tức
        this.cdr.markForCheck();

        // Trì hoãn việc load dữ liệu để trình duyệt cập nhật UI tab trước
        setTimeout(() => {
            this.loadData();
            this.filterSteps();
            this.cdr.markForCheck();
    ...
    onPrint(rowData: any) {
        ...
        this.paymentService.getDetail(rowData.ID).subscribe({
            next: (response) => {
                const dataPrint = {
