import { Component, ViewEncapsulation, ViewChild, TemplateRef, ElementRef, Input, Output, EventEmitter } from '@angular/core';
    ...
    initTable(): void {
        this.table = new Tabulator(this.tableElement.nativeElement, {
            ...
            columns: [
                ...
                {
                    title: 'Tình trạng giao hàng (Nhận đủ/ Thiếu)', field: 'DeliveryStatus', width: 200, editor: "list", editorParams: {
    ...
    initializeTabFormData(): void {
        ...
        for (let i = 0; i < this.groupedData.length; i++) {
            ...
            if (mainData) {
                ...
                const customerObj = this.customers.find(x => x.CustomerName === mainData.CustomerName);
                const adminWarehouseObj = this.employeesAndDepartments.find(x => x.Employee.FullName === mainData.AdminWarehouseName);
                const employeeObj = this.employeesAndDepartments.find(x => x.Employee.FullName === mainData.FullName);
                ...
                defaultForm.adminWarehouse = adminWarehouseObj ? adminWarehouseObj.Employee.ID : null;
            }
