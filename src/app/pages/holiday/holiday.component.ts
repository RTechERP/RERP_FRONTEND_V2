...
export class HolidayComponent implements OnInit {
  ...
  private initializeTable(): void {
    this.holidayTabulator = new Tabulator('#tb_holiday', {
      ...
      height: '85vh',
      columns: [
        { title: 'Ngày', field: 'HolidayDate', hozAlign: 'center', headerHozAlign: 'center',
          formatter: (cell) => {
            const value = cell.getValue();
            return value ? DateTime.fromISO(value).toFormat('dd/MM/yyyy') : '';
          }
        },
        { title: 'Tên', field: 'HolidayName', hozAlign: 'center', headerHozAlign: 'center'},
        { title: 'Loại', field: 'TypeHolidayText', hozAlign: 'center', headerHozAlign: 'center'},
      ]
    });
  ...
  openAddModal() {
    ...
    const modal = new (window as any).bootstrap.Modal(document.getElementById('addHolidayModal'));
    modal.show();
  }
  ...
  openDeleteModal() {
    ...
    this.selectedHoliday = selectedRows[0].getData();
    const formattedDate = DateTime.fromISO(this.selectedHoliday.HolidayDate).toFormat('dd/MM/yyyy');
    this.modal.confirm({
  ...
  openScheduleWorkModal() {
    const modal = new (window as any).bootstrap.Modal(document.getElementById('scheduleWorkModal'));
    modal.show();
  }
}
