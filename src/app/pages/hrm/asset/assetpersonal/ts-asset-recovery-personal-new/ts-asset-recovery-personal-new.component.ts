...
export class TsAssetRecoveryPersonalNewComponent
  implements OnInit, AfterViewInit
{
      ...
      .subscribe((respon: any) => {
        ...
        if (this.tbAssetPersonModal) {
          this.tbAssetPersonModal.setData(
            this.cleanAssetRowsForCreate(this.assetPersonals)
          );
  ...
  getAssetRecoveryPersonals() {
    ...
    this.tsAssetRecoveryPersonalService
      .getAssetRecoveryPersonal(req)
      .subscribe((res: any) => {
  ...
  drawTbRecoveryPersonal() {
    this.tbAssetRecoveryPersonal = new Tabulator('#dataTbRecovery3232', {
      ...
      ajaxRequestFunc: (url, config, params) => {
        ...
        return this.tsAssetRecoveryPersonalService
          .getAssetRecoveryPersonal(request)
          .toPromise();
      },
      ...
      columns: [
        ...
        {
          title: 'EmployeeReturnID',
          field: 'EmployeeReturnID',
          visible: false,
        },
        {
          title: 'EmployeeRecoveryID',
        ...
        },
        {
          ...
          formatter: (c) =>
            c.getValue()
              ? new Date(c.getValue()).toLocaleDateString('vi-VN')
              : '',
        },
        {
          title: 'Ngày duyệt HR',
        ...
        {
          title: 'Thu hồi từ',
          field: 'FullNameReturn',
          headerHozAlign: 'center',
        },
        {
          title: 'Người thu hồi',
          field: 'FullNameRecovery',
          headerHozAlign: 'center',
        },
    ...
    this.tbAssetRecoveryPersonal.on(
      'rowDblClick',
  ...
  validateRecoveryForm(): boolean {
    ...
    if (!this.employeeReturnID) {
      this.notification.warning(
        NOTIFICATION_TITLE.warning,
        'Vui lòng chọn nhân viên thu hồi từ!'
      );
      isValid = false;
    }
  ...
  saveRecoveryPersonal() {
    ...
    if (selectAssetModal.length === 0) {
      this.notification.warning(
        NOTIFICATION_TITLE.warning,
        'Không có tài sản nào trong danh sách để lưu'
      );
      return;
    }
  ...
  generateTSAssetCode(date?: string): void {
    const d =
      date || this.recoveryDate || new Date().toISOString().split('T')[0];
    this.recoveryDate = d;

    this.tsAssetRecoveryPersonalService.getTSTHCode(d).subscribe({
      next: (res) => {
        (this.TSTHcode = res.data), console.log('TSTHcode', this.TSTHcode);
      },
  ...
  openEditModal() {
    ...
    const selectedRecovery =
      this.tbAssetRecoveryPersonal?.getSelectedData()?.[0];
    ...
    this.recoveryDate = DateTime.fromISO(
      selectedRecovery.DateRecovery
    ).toFormat('yyyy-MM-dd');
    this.employeeReturnID = selectedRecovery.EmployeeReturnID;
    this.employeeRecoveryID = selectedRecovery.EmployeeRecoveryID;
    const employeereturn = this.emPloyeeLists.find(
      (e) => e.ID === this.employeeReturnID
    );
    const employeerecovery = this.emPloyeeLists.find(
      (e) => e.ID === this.employeeRecoveryID
    );
      ...
      if (this.tbAssetRecoveryDetail) {
        ...
        this.tbAssetRecoveryDetail = new Tabulator('#dataTbRecoveryDetail', {
          ...
          columns: [
            ...
            {
              title: 'TSRecoveryAssetPersonalID',
              field: 'TSRecoveryAssetPersonalID',
              hozAlign: 'center',
              width: 60,
              visible: false,
            },
  ...
  drawTbAssetManagementModal() {
    if (this.tbAssetPersonModal) {
      ...
      this.tbAssetPersonModal = new Tabulator(
        '#dataTbAssetRecoveryManagement',
        {
  ...
  onEmployeeChange(employeeID: number): void {
    ...
    this.selectedEmployeeReturnID =
      this.emPloyeeLists.find((emp) => emp.ID === employeeID) || null;
    this.selectedEmployeeRecoveryID =
      this.emPloyeeLists.find((emp) => emp.ID === employeeID) || null;
  }
      ...
      switch (action) {
        ...
        case 'Delete':
          ...
          if (row.IsApproveHR === true) {
            this.notification.warning(
              NOTIFICATION_TITLE.warning,
              'Tài sản đã được HR duyệt, không thể xóa.'
            );
            return false;
          }
          break;
      ...
      case 'HR_APPROVE':
        updatePayload = {
          tSRecoveryAssetPersonal: {
            id,
            isApproveHR: true,
            DateApprovedHR: today.toISOString().split('T')[0],
            EmployeeReturnID: employeeId,
          },
        };
        break;
      case 'HR_CANCEL':
      ...
      case 'PERSONAL_APPROVE':
        updatePayload = {
          tSRecoveryAssetPersonal: {
            ...
            DateApprovedPersonalProperty: today.toISOString().split('T')[0],
            EmployeeReturnID: employeeId,
          },
        };
        break;
  ...
  updateApprovePeronal(
    ...
    this.tsAssetAllocationPersonalService
      .SaveApprovePerson(updatePayload)
      .subscribe({
        next: (res) => {
          if (res.status === 1) {
            this.notification.success(NOTIFICATION_TITLE.success, 'Thành công');
