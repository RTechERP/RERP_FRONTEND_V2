...
@Component({
  selector: 'app-contract-transfer-review-hr',
  standalone: true,
  imports: [
  ...
  templateUrl: './contract-transfer-review-hr.component.html',
  styleUrl: './contract-transfer-review-hr.component.css',
})
export class ContractTransferReviewHrComponent implements OnInit, OnDestroy {
  ...
  initMenu(): void {
    ...
    this.menuItems = [
      ...
      },
      {
        ...
        visible: true, // Nội dung chữ đã đổi động
        command: () => this.onEdit(),
      },
      {
        label: 'Xóa',
      ...
      // ─── HR xác nhận ────────────────────────────────────────────────────────
      {
        ...
        visible: this.permissionService.hasPermission('N1,N56') || this.permissionService.hasPermission('N2'),
        items: [
          {
            label: 'HR duyệt',
  ...
  get rowActionBtn(): { label: string; icon: string; nzType: string } {
    return { label: 'Sửa', icon: 'edit', nzType: 'primary' };
  }
