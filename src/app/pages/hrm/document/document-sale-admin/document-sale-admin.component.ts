...
export class DocumentSaleAdminComponent implements OnInit, AfterViewInit {
    ...
    initGrid(): void {
        ...
        this.columnDefinitions = [
            ...
            {
                id: 'Code', name: 'Mã văn bản', field: 'Code', width: 150, sortable: true, filterable: true,
                filter: {
                    model: Filters['multipleSelect'],
                    collection: [],
                    filterOptions: { autoAdjustDropHeight: true, autoAdjustDropPosition: true } as MultipleSelectOption
            ...
            {
                id: 'NameDocument', name: 'Tên văn bản', field: 'NameDocument', width: 300, sortable: true, filterable: true,
                filter: {
                    model: Filters['multipleSelect'],
                    collection: [],
                    filterOptions: { autoAdjustDropHeight: true, autoAdjustDropPosition: true } as MultipleSelectOption
            ...
            {
                id: 'DatePromulgate', name: 'Ngày ban hành', field: 'DatePromulgate', width: 120, sortable: true, filterable: true,
                formatter: formatDate,
                type: 'date',
                filter: { model: Filters['compoundDate'] }
            },
            {
                id: 'DateEffective', name: 'Ngày hiệu lực', field: 'DateEffective', width: 120, sortable: true, filterable: true,
                formatter: formatDate,
                type: 'date',
                filter: { model: Filters['compoundDate'] }
            },
            {
                id: 'DepartmentCode', name: 'Mã bộ phận', field: 'DepartmentCode', width: 100, sortable: true, filterable: true,
                filter: {
                    model: Filters['multipleSelect'],
                    collection: [],
                    filterOptions: { autoAdjustDropHeight: true, autoAdjustDropPosition: true } as MultipleSelectOption
            ...
            {
                id: 'DepartmentName', name: 'Bộ phận phát hành', field: 'DepartmentName', width: 180, sortable: true, filterable: true,
                filter: {
                    model: Filters['multipleSelect'],
                    collection: [],
                    filterOptions: { autoAdjustDropHeight: true, autoAdjustDropPosition: true } as MultipleSelectOption
            ...
            {
                id: 'EmployeeSignCode', name: 'Mã người ký', field: 'EmployeeSignCode', width: 100, sortable: true, filterable: true,
                filter: {
                    model: Filters['multipleSelect'],
                    collection: [],
                    filterOptions: { autoAdjustDropHeight: true, autoAdjustDropPosition: true } as MultipleSelectOption
            ...
            {
                id: 'EmployeeSignName', name: 'Tên người ký', field: 'EmployeeSignName', width: 150, sortable: true, filterable: true,
                filter: {
                    model: Filters['multipleSelect'],
                    collection: [],
                    filterOptions: { autoAdjustDropHeight: true, autoAdjustDropPosition: true } as MultipleSelectOption
            ...
            {
                id: 'AffectedScope', name: 'Phạm vi áp dụng', field: 'AffectedScope', width: 150, sortable: true, filterable: true,
                filter: {
                    model: Filters['multipleSelect'],
                    collection: [],
                    filterOptions: { autoAdjustDropHeight: true, autoAdjustDropPosition: true } as MultipleSelectOption
    ...
    initGridFile(): void {
        this.columnDefinitionsFile = [
            {
                id: 'FileName', name: 'Tên file', field: 'FileName', width: 300, sortable: true, filterable: true,
    ...
    getSelectedData(): any[] {
        ...
        return selectedRowIndexes.map((idx: number) => this.angularGrid.dataView?.getItem(idx)).filter(Boolean);
    }
    ...
    applyDistinctFilters(): void {
        ...
        columnsToDistinct.forEach(field => {
            const distinctValues = Array.from(new Set(data.map(item => item[field]).filter(val => val !== null && val !== undefined && val !== '')));
            const collection = distinctValues.sort().map(val => ({ value: String(val), label: String(val) }));
    ...
    getDocumentFileByID(id: number) {
        this.documentService.getDocumentFileByID(id).subscribe((response: any) => {
            ...
            this.datasetFile = this.documentFileData.map((item: any, index: number) => ({
                ...item,
                id: item.ID || index + 1
            }));
        });
    ...
    exportExcel(): void {
        ...
        const filteredData = this.angularGrid?.dataView?.getFilteredItems() || this.dataset;
            ...
            nzOnOk: () => {
                this.documentService.saveDocumentFile(payloads).subscribe({
                    next: (res) => {
                        if (res.status === 1) {
                            this.notification.success('Thông báo', 'Đã xóa thành công!');
                            this.datasetFile = this.datasetFile.filter(f => f.ID !== dataSelect[0].ID);
                            this.data = [];
                        } else {
                            this.notification.warning(
                                'Thông báo',
