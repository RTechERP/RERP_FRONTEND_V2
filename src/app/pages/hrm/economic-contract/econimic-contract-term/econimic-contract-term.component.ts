...
import { EconimicContractTermFormComponent } from './econimic-contract-term-form/econimic-contract-term-form.component';

@Component({
    standalone: true,
    selector: 'app-econimic-contract-term',
    imports: [
        CommonModule,
        FormsModule,
        NgbModalModule,
    ...
    styleUrl: './econimic-contract-term.component.css'
})
export class EconimicContractTermComponent implements OnInit {
    ...
    onCreate() {
        const modalRef = this.ngbModal.open(EconimicContractTermFormComponent, {
            size: 'lg',
            backdrop: 'static',
            keyboard: false,
            centered: true,
    ...
    onDelete() {
        ...
        if (selectedIds.length === 1) {
            ...
            content = `Bạn chắc chắn muốn xóa điều khoản <b>${rowData.TermCode} - ${rowData.TermName}</b>?`;
        } else {
            content = `Bạn chắc chắn muốn xóa <b>${selectedIds.length}</b> điều khoản đã chọn?`;
        }
        ...
        this.nzModal.confirm({
            ...
            nzCancelText: 'Hủy',
            nzOnOk: () => {
                return this.economicContractService.deleteEconomicContractTerm(selectedIds)
                    .toPromise()
                    .then((res: any) => {
                        if (res?.status === 1) {
