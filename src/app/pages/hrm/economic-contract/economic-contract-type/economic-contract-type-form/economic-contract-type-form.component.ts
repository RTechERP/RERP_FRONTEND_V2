...
@Component({
    standalone: true,
    selector: 'app-economic-contract-type-form',
    imports: [
        CommonModule,
        FormsModule,
        ReactiveFormsModule,
    ...
    templateUrl: './economic-contract-type-form.component.html',
    styleUrl: './economic-contract-type-form.component.css'
})
export class EconomicContractTypeFormComponent implements OnInit {
    formGroup!: FormGroup;
    @Input() dataInput: any;
    ...
    save(isClose: boolean = true) {
        ...
        this.economicContractService.saveEconomicContractType(payload).subscribe({
            next: (response: any) => {
                if (response?.status === 1) {
                    this.notification.success(NOTIFICATION_TITLE.success, 'Lưu loại hợp đồng thành công');
