...
export class VehiceDetailComponent implements OnInit, AfterViewInit {
  ...
  saveVehiceType(){
    ...
    data.forEach((item: any) => {
      ...
      if (item.vehiceName && item.vehiceName.trim() !== '') {
        vehicleNames.push(item.vehiceName.trim());
      } else if (item.vehiceType) {
        const vehicle = this.vehiceList.find(v => v.ID === item.vehiceType);
        if (vehicle) {
          vehicleNames.push(vehicle.vehiceType);
        }
      }
    });
    
    // Trả về dữ liệu khi đóng modal
