  ...
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  private drawTbenf(container: HTMLElement): void {
    ...
    this.tb_ENF = new Tabulator(container, {
      ...
      ajaxResponse: (url: string, params: any, res: any) => {
        let totalPage = 0;
  ...
  selectAllRows(): void {
    ...
    // Removed unnecessary notification for common action
  }
  ...
  private showNotification(type: 'success' | 'error' | 'warning' | 'info', title: string, message: string): void {
    this.notification[type](title, message, { nzStyle: { fontSize: '0.75rem' } });
  }
  // #endregion
  ...
  addenf(): void {
    ...
    modalRef.result.then((result) => {
      if (result?.action === 'save') {
        this.searchenf();
        this.showNotification('success', 'Thông báo', 'Thêm ENF thành công!');
      }
    });
  }
  ...
  editenf(): void {
    ...
    if (!enfToEdit) {
      this.showNotification('error', 'Thông báo', 'Vui lòng chọn 1 ENF cần sửa!');
      return;
    }
    ...
    modalRef.result.then((result) => {
      if (result?.action === 'save') {
        this.searchenf();
        this.showNotification('success', 'Thông báo', 'Sửa ENF thành công!');
      }
    });
  }
  ...
  deleteenf(): void {
    ...
    if (selectedRows.length === 0) {
      this.showNotification('error', 'Thông báo', 'Vui lòng chọn ít nhất 1 ENF cần xóa!');
      return;
    }
  ...
  private handleDeleteComplete(
    ...
    if (successCount > 0 || failedCount > 0) {
      // Combined notification for better UX
      const message = failedCount > 0 
        ? `Đã xóa ${successCount}/${totalCount} bản ghi ENF thành công! ${failedCount} bản ghi thất bại.`
        : `Đã xóa ${successCount}/${totalCount} bản ghi ENF thành công!`;
      
      const type = failedCount > 0 ? 'warning' : 'success';
      this.notification[type]('Thông báo', message, { nzStyle: { fontSize: '0.75rem' } });
  ...
  approvedTBP(): void {
    const selectedRows = this.getSelectedRows();
    if (selectedRows.length === 0) {
      this.showNotification('error', 'Thông báo', 'Vui lòng chọn ít nhất 1 ENF cần duyệt!');
      return;
    }
    this.nzModal.confirm({
  ...
  private confirmApproveTBP(selectedRows: any[]): void {
    ...
    const approveNext = (index: number) => {
      if (index >= selectedRows.length) {
        // Combined notification for better UX
        const message = failedCount > 0 
          ? `TBP đã duyệt ${successCount}/${selectedRows.length} bản ghi thành công! ${failedCount} bản ghi thất bại.`
  ...
  private confirmApproveHR(selectedRows: any[]): void {
    ...
    const approveNext = (index: number) => {
      if (index >= selectedRows.length) {
        // Combined notification for better UX
        const message = failedCount > 0 
          ? `HR đã duyệt ${successCount}/${selectedRows.length} bản ghi thành công! ${failedCount} bản ghi thất bại.`
  ...
  cancelApprovedHR(): void {
    const selectedRows = this.getSelectedRows();
    if (selectedRows.length === 0) {
      this.showNotification('error', 'Thông báo', 'Vui lòng chọn ít nhất 1 ENF cần hủy duyệt HR!');
      return;
    }
    const confirmMsg =
  ...
  private confirmCancelApprovedHR(selectedRows: any[]): void {
    ...
    const cancelNext = (index: number) => {
      if (index >= selectedRows.length) {
        // Combined notification for better UX
        const message = failedCount > 0 
          ? `HR đã hủy duyệt ${successCount}/${totalCount} bản ghi thành công! ${failedCount} bản ghi thất bại.`
  ...
  cancelApproveTBP(): void {
    const selectedRows = this.getSelectedRows();
    if (selectedRows.length === 0) {
      this.showNotification('error', 'Thông báo', 'Vui lòng chọn ít nhất 1 ENF cần hủy duyệt!');
      return;
    }
    this.nzModal.confirm({
  ...
  private confirmCancelApproveTBP(selectedRows: any[]): void {
    ...
    const cancelNext = (index: number) => {
      if (index >= selectedRows.length) {
        // Combined notification for better UX
        const message = failedCount > 0 
          ? `TBP đã hủy duyệt ${successCount}/${selectedRows.length} bản ghi thành công! ${failedCount} bản ghi thất bại.`
  ...
  exportExcel(): void {
    if (!this.tb_ENF) {
      this.showNotification('error', 'Thông báo', 'Bảng dữ liệu chưa sẵn sàng!');
      return;
    }
    ...
    if (allData.length === 0) {
      this.showNotification('warning', 'Thông báo', 'Không có dữ liệu để xuất!');
      return;
    }
    ...
    try {
      ...
      // Immediate notification after successful export
      this.showNotification('success', 'Thông báo', `Đã xuất ${allData.length} bản ghi ENF ra file Excel thành công!`);
    } catch (error) {
      console.error('Export Excel error:', error);
      this.showNotification('error', 'Thông báo', 'Lỗi khi xuất file Excel: ' + (error instanceof Error ? error.message : 'Lỗi không xác định'));
    }
  }
