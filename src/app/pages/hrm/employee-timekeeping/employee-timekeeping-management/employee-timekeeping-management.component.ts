  ...
  implements OnInit, AfterViewInit
{
  ...
  ngOnInit() {
    // Ưu tiên ID nhận từ modal
    const idFromInput = Number(this.etData?.ID || 0);
    if (idFromInput > 0) {
      this.etId = idFromInput;
      this.masterId = idFromInput;
      this.prefillFromETById(idFromInput);
      return;
    }

    // Fallback: nếu component được dùng như page, lấy từ route snapshot
    const idFromRoute = Number(this.route.snapshot.paramMap.get('id') || 0);
    this.etId = idFromRoute;
    this.masterId = idFromRoute;

    if (idFromRoute > 0) {
      this.prefillFromETById(idFromRoute);
    } else {
      // Không có ID -> load mặc định
  ...
  private prefillFromETById(id: number) {
    ...
    this.etService.getETById(id).subscribe({
      ...
      error: (e) => console.error('prefillFromETById error:', e),
    });
  }
  ...
  exportToExcel(): void {
    ...
    this.notification.info('Đang xuất Excel', 'Đang lấy dữ liệu MT & DT...');
  ...
  onDepartmentChange(): void {
    // load lại danh sách nhân viên theo phòng ban (giữ nguyên phần lọc như bạn đang có)
    if (this.selectedDepartment) {
      this.etService.getEmployee().subscribe({
        next: (res) => {
          if (res?.status === 1 && res.data) {
            const filtered = res.data.filter(
              (e: any) => e.DepartmentID === this.selectedDepartment
            );
  ...
  onUpdateOne(): void {
    ...
    if (!masterId) {
      this.notification.error('Lỗi', 'Không xác định MasterID (etId).');
      return;
    }
    ...
    if (!employeeId) {
      this.notification.info(
        'Thông báo',
        'Vui lòng chọn nhân viên (trong bộ lọc hoặc chọn 1 dòng ở MT/DT).'
      );
      return;
    }

    if (!confirm(`Bạn có chắc muốn cập nhật công cho nhân viên [${fullName}]?`))
      return;
    ...
    this.etService
      ...
      .subscribe({
        ...
        error: () => {
          this.isLoadTable = false;
          this.notification.error('Lỗi', 'Không thể cập nhật công nhân viên');
        },
      });
  }
  ...
  onUpdateAll(): void {
    ...
    if (
      !confirm(
        `Bạn có chắc muốn cập nhật lại bảng chấm công tháng ${p.month}/${p.year} cho TẤT CẢ nhân viên?`
      )
    )
      return;
    ...
    this.etService.getEmployee().subscribe({
      next: (response) => {
        if (response && response.status === 1 && response.data) {
          ...
          const groupedEmployees = this.groupEmployeesByDepartment(
            response.data
          );
          this.employees = groupedEmployees;
  ...
  private reloadBothFast(): void {
    ...
    forkJoin({
      ...
      error: (err) => {
        console.error('reloadBothFast error:', err);
        this.isLoadTable = false;
        // clear nhẹ cho chắc
        this.tb_MT?.setData([]);
        this.tb_DT?.setData([]);
        this.notification.error('Lỗi', 'Không thể tải dữ liệu MT/DT');
      },
    });
  }
  ...
  drawTbMT(container: HTMLElement) {
    if (!container) {
      console.error('Container for table MT not found');
      return;
    }
    ...
    try {
      ...
      this.tb_MT.on('dataLoading', () => {
        console.log('MT table data loading...');
        this.isLoadTable = true;
      });

      this.tb_MT.on('dataLoaded', (data: any) => {
        console.log('MT table data loaded:', data.length, 'records');
        this.isLoadTable = false;
      });

      this.tb_MT.on('dataLoadError', (error: any) => {
        console.error('MT Data Load Error:', error);
        this.isLoadTable = false;
        this.notification.error('Lỗi', 'Không thể tải dữ liệu bảng chấm công');
      });
      ...
      this.tb_MT.on('tableBuilt', () => {
        console.log('MT table built successfully');
      ...
      // Thêm event để debug
      this.tb_MT.on('renderComplete', () => {
        console.log(
          'Table render complete, data count:',
          this.tb_MT.getDataCount()
        );
      });
    } catch (error) {
      console.error('Error creating MT table:', error);
      this.isLoadTable = false;
      this.notification.error('Lỗi', 'Không thể khởi tạo bảng chấm công');
    }
  }
  ...
  drawTbDT(container: HTMLElement) {
    if (!container) {
      console.error('Container for table DT not found');
      return;
    }
    ...
    try {
      ...
      console.error('Error creating DT table:', err);
    }
  }
    ...
    this.etService
      ...
      .subscribe({
        next: (res) => {
          ...
          console.log('DT normalized rows:', rows.length);
          this.tb_DT.setData(rows);
        },
        error: (err) => {
          this.isLoadTable = false;
          this.tb_DT.setData([]);
          this.notification.error('Lỗi', 'Không thể tải dữ liệu chi tiết công');
  ...
  refreshData(): void {
    if (this.isTableBuilt && this.tb_MT) {
      this.loadMTData();
      this.notification.info('Thông báo', 'Đã làm mới dữ liệu');
    }
  }
