...
export class EmployeeApproveComponent implements OnInit, AfterViewInit {
  ...
  initializeEmployeeApproveTable() {
    ...
    this.employeeApproveTabulator.on('rowClick', (e: any, row: any) => {
      this.employeeApproveTabulator.deselectRow();
      row.select();
    });
  }
  ...
  onSearchEmployee(event: any) {
    ...
    if (keyword != "") {
      this.employeeApproveTabulator.setFilter([
        [
      ...
      this.employeeApproveTabulator.clearFilter;
      this.loadEmployeeApprove();
      this.initializeEmployeeApproveTable();
    }
  }
  ...
  deleteEmployeeApprove() {
    ...
    if (selectedRows.length === 0) {
      this.notification.warning(NOTIFICATION_TITLE.warning, 'Vui lòng chọn người duyệt cần xóa');
      return;
    }
