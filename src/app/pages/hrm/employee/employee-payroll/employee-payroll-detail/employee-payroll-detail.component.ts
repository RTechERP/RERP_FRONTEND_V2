...
export class EmployeePayrollDetailComponent implements OnInit {
  ...
  onSubmit() {
    if (this.validateForm.valid) {
      ...
      this.employeePayrollService.getCheckExistEmployeePayroll(id, month, year)
        .subscribe({
          next: (data) => {
  ...
  checkExistEmployeePayroll() {
    ...
    const month = this.validateForm.get('_Month')?.value ?? 0;
    const year = this.validateForm.get('Year')?.value.getFullYear() ?? 0;
    this.employeePayrollService.getCheckExistEmployeePayroll(id, month, year).subscribe({
      next: (data) => {
        if (data.status == 1) {
          if (data.data == null) {
            this.isValid = true;
  ...
  fillData(employeePayrollID: number) {
    if (employeePayrollID != 0) {
      this.employeePayrollService.getEmployeePayrollByID(employeePayrollID).subscribe({
        next: (data) => {
  ...
  saveData() {
    ...
    this.employeePayrollService.postSaveEmployeePayroll(this.validateForm.getRawValue()).subscribe({
      next: (data) => {
        if (data.status == 1) {
          this.notification.create(
            'success',
