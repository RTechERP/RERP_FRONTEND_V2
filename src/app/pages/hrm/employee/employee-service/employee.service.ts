...
export class EmployeeService {
  ...
  getEmployees(): Observable<any> {
    return this.http.get(
      this._url + 'Employee?status=0&departmentID=0&keyword='
    );
  }
  ...
  saveEmployee(employee: any): Observable<any> {
    return this.http.post<any>(this._url + 'Employee/save-data', employee);
  }
  ...
  printContract(id: number): Observable<any> {
    return this.http.get<any>(
      this._url + `EmployeeContract/${id}/print-contract`
    );
  }

  generateWordDocument(data: any): Observable<any> {
    return this.http.post(this._url + 'EmployeeContract/generate', data, {
      responseType: 'blob',
    });
  }
  ...
  checkEmployeeCodes(codesToCheck: { Code: string }[]): Observable<any> {
    return this.http.post<any>(
      this._url + 'Employee/check-codes',
      codesToCheck
    );
  }
