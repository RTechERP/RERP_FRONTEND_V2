...
export class HolidayComponent implements OnInit, AfterViewInit {
  ...
  private initializeTable(datasource: any): void {
    ...
    this.holidayTabulator.on('rowClick', (e: any, row: any) => {
      ...
      if (clickedField !== 'select') {
        ...
        this.holidayTabulator.deselectRow();
        row.select();
      }
    });
  }
  ...
  onSubmit() {
    ...
    const toDateString = (date: any) =>
      date ? DateTime.fromJSDate(new Date(date)).toFormat("yyyy-MM-dd") : null;
    if (this.holidayForm.valid) {
      const formData = {
        ...this.holidayForm.value,
        HolidayDate: toDateString(this.holidayForm.value.HolidayDate),
  ...
  openDeleteModal() {
    ...
    this.modal.confirm({
      ...
      nzContent: `Bạn có chắc chắn muốn xóa ngày nghỉ đã chọn không?`,
      nzOkText: 'Xóa',
      nzOkType: 'primary',
      nzOkDanger: true,
      nzOnOk: () => {
  ...
  @ViewChild(EmployeeScheduleWorkComponent) employeeScheduleWorkComponent!: EmployeeScheduleWorkComponent
  openScheduleWorkModal() {
    const modal = new (window as any).bootstrap.Modal(
      document.getElementById('scheduleWorkModal')
    );
    modal.show();
    this.employeeScheduleWorkComponent.ngOnInit();
  }
}
