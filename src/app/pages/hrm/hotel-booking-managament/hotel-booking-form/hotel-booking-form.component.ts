...
export class HotelBookingFormComponent implements OnInit, OnChanges {
  ...
  addProposal(data?: any): void {
    const proposalGroup = this.fb.group({
      ...
      Distance: this.fb.control(data?.Distance ?? data?.distance ?? ''),
      Area: this.fb.control<number | null>(data?.Area ?? data?.area ?? null),
      Convenience: this.fb.control(data?.Convenience ?? data?.convenience ?? ''),
