...
export class HomeLayoutCandidateComponent implements OnInit, OnDestroy, OnChanges {
    ...
    patchForm(data: any) {
        ...
        if (mainForm) {
            ...
            // Cập nhật trạng thái IsComplete (chỉ áp dụng cho ứng viên, HR luôn được chỉnh sửa)
            ...
            if (mainForm.FileName) {
                // Chỉ download nếu chưa có preview hoặc không phải là file đang chọn local
                // (Khi chuyển candidate, loadInitialData đã xóa imagePreview nên sẽ luôn vào đây)
                    ...
                    if (candidateStr) {
                        ...
                        this.form.patchValue({
                            ...
                            Gender: this.normalizeGender(
                                candidate.Gender ?? candidate.GenderID ?? candidate.Sex ?? candidate.GioiTinh ?? candidate.GenderText
                            ),
