...
export class ImportExcelComponent implements OnInit, AfterViewInit {
  ...
  async readExcelData(workbook: ExcelJS.Workbook, sheetName: string) {
    ...
    try {
      ...
      console.log(
        `Đã đọc ${data.length} dòng dữ liệu không trống từ sheet (hiển thị preview).`
      );
      console.log(
        `Tìm thấy ${validRecords} bản ghi hợp lệ (bắt đầu từ STT số).`
      ); // Log rõ ràng hơn
  ...
  saveExcelData() {
    ...
    console.log(
      'Tổng số bản ghi cần lưu (trước lọc):',
      this.dataTableExcel.length
    );
    ...
    console.log(
      'codesToCheck (dữ liệu gửi đi kiểm tra, đã lọc STT số):',
      codesToCheck
    );
    ...
    this.officesupplyService.checkProductCodes(codesToCheck).subscribe({
      next: (response: any) => {
        ...
        const existingProducts =
          response.data && Array.isArray(response.data.existingProducts)
            ? response.data.existingProducts
            : [];
        console.log(
          'existingProducts (sau khi xử lý response):',
          existingProducts
        );
        ...
        const processedData = validDataToSave.map((row, index) => {
          const existingProduct = existingProducts.find(
            (p: any) => p.CodeRTC === row.CodeRTC && p.CodeNCC === row.CodeNCC
          );
        ...
        console.log(
          'processedData (dữ liệu cuối cùng gửi đi lưu, đã lọc STT số):',
          processedData
        );
        ...
        if (processedData.length === 0) {
          this.notification.info(
            'Thông báo',
            'Không có sản phẩm hợp lệ để tiến hành lưu.'
          );
        ...
        const saveProductWithDelay = (index: number) => {
          ...
          console.log(
            `Gửi lưu sản phẩm ${index + 1}/${totalProductsToSave}:`,
            product
          );
          ...
          setTimeout(() => {
            this.officesupplyService.adddata(product).subscribe({
              next: (response: any) => {
                console.log(
                  `Response từ adddata cho sản phẩm ${index + 1}:`,
                  response
                );
                ...
                this.displayProgress = Math.round(
                  (completedRequests / totalProductsToSave) * 100
                );
      ...
      error: (err) => {
        ...
        this.notification.error(
          NOTIFICATION_TITLE.error,
          'Có lỗi xảy ra khi kiểm tra mã sản phẩm từ database!'
        );
    ...
    this.officesupplyService.getdata(this.searchText).subscribe({
      next: (res) => {
        ...
        if (this.lastAddedIdProduct) {
          const newItem = this.lstVP.find(
            (item) => item.ID === this.lastAddedIdProduct
          );
          if (newItem) {
            // Tách sản phẩm mới ra khỏi danh sách
            this.lstVP = this.lstVP.filter(
              (item) => item.ID !== this.lastAddedIdProduct
            );
