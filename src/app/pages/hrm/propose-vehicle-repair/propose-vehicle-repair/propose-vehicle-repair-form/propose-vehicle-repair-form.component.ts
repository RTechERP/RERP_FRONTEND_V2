...
export class ProposeVehicleRepairFormComponent implements OnInit, AfterViewInit {
...
onTabChange(idx: number) {
  this.activeTab = idx;                // <— thiếu dòng này
  if (idx === 1) setTimeout(() => this.ensureNccTableReady(), 0);
}
  ...
  private initNccTable(data: ProposeDetail[] = []) {
    ...
    this.nccTable = new Tabulator(el, {
      ...
      data, layout: 'fitColumns', reactiveData: true, height: '52vh',
    ...
    `,
          cellClick: (_e, cell) => {
            ...
            // Nếu là bản ghi đã có trong DB thì đánh dấu xoá mềm
            if (d.ID && d.ID > 0) {
              // tránh trùng ID
        ...
        { title: 'STT', field: 'STT', width: 64, hozAlign: 'center', headerHozAlign: 'center' },
        { title: 'VehicleRepairProposeID', field: 'VehicleRepairProposeID', width: 64, hozAlign: 'center', headerHozAlign: 'center', visible: false },
        { title: 'ID', field: 'ID', width: 64, hozAlign: 'center', headerHozAlign: 'center', visible: false },
        { title: 'Tên NCC', field: 'GaraName', width: 200, editor: 'input' },
        { title: 'Địa chỉ NCC', field: 'AddressGara', width: 240, editor: 'input' },
        { title: 'SĐT NCC', field: 'SDTGara', width: 130, editor: 'input' },
        { title: 'Số lượng', field: 'Quantity', width: 110, hozAlign: 'right', headerHozAlign: 'center', editor: 'number', editorParams: { min: 0, step: 1 } },
        { title: 'Đơn vị', field: 'Unit', width: 100, editor: 'input' },
        {
          title: 'Đơn giá', field: 'UnitPrice', width: 130, hozAlign: 'right', headerHozAlign: 'center',
          editor: 'number', editorParams: { min: 0, step: 1000 },
          formatter: 'money', formatterParams: { decimal: ',', thousand: '.', symbol: 'đ', symbolAfter: true, precision: 0 }
        },
        {
          title: 'Thành tiền', field: 'TotalPrice', width: 140, hozAlign: 'right', headerHozAlign: 'center',
          mutator: (_v, d: ProposeDetail) => Number(d.Quantity || 0) * Number(d.UnitPrice || 0),
          formatter: 'money', formatterParams: { decimal: ',', thousand: '.', symbol: 'đ', symbolAfter: true, precision: 0 }
        },
    ...
    this.nccTable.on('cellEdited', (cell) => {
      ...
      if (f === 'Quantity' || f === 'UnitPrice') {
        const row = cell.getRow();
        const d = row.getData() as ProposeDetail;
        row.update({ TotalPrice: Number(d.Quantity || 0) * Number(d.UnitPrice || 0) });
      }
    });
  ...
  addDetailRow() {
    ...
    this.nccTable.addRow({
      ID: 0,
      STT: next, IsApprove: 0, ApproveName: '', GaraName: '', AddressGara: '', SDTGara: '',
      Quantity: 1, Unit: '', UnitPrice: 0, TotalPrice: 0, Note: '', IsDeleted: false
    }, true);
...
private getNccDetails(): ProposeDetail[] {
  if (this.nccTable) return (this.nccTable.getData() as ProposeDetail[]) || [];
  ...
  private loadDetails(id: number) {
    this.deletedDetails = [];
    this.proposeVehicleRepairService.getProposeVehicleRepairDetail(id).subscribe({
      next: (res: any) => {
  ...
  private hydrateDetailsBuffer() {
    ...
    if (mapped.length) {
      ...
      if (!this.dataInput?.ID && this.nccTable.getDataCount() === 0) this.addDetailRow();
    }
  }
  ngOnDestroy(): void {
    try { this.nccTable?.destroy(); } catch { }
  }
 ...
 if (this.formGroup.invalid) {
  ...
  ['DatePropose', 'TimeStartRepair', 'TimeEndRepair'].forEach(key => {
    ...
    if (val) {
      ...
      if (!(d instanceof Date) || isNaN(d.getTime())) invalidDates.push(key);
    }
  });
