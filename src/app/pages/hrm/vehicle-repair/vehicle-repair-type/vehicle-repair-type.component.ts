...
import { TabulatorFull as CellComponent, ColumnDefinition, RowComponent } from 'tabulator-tables';
 ...
 if (this.vehicleRepairTypeTable) {
      ...
      this.vehicleRepairTypeTable = new Tabulator('#vehicleRepairType', {
        ...
        rowClick: (e: MouseEvent, row: RowComponent) => {
          this.vehicleRepairTypeTable!.getSelectedRows().forEach(r => r.deselect());
          row.select();
  ...
  modalRef.result.then(() => this.getRepairType(), () => {});
}
...
deleteType() {
  ...
  this.nzModal.confirm({
    ...
    nzContent: `Bạn có chắc chắn muốn xóa loại <strong>"${rowData.RepairTypeName || 'N/A'}"</strong> không?`,
    nzOkText: 'Xóa',
    nzOkType: 'primary',
    nzOkDanger: true,
    nzCancelText: 'Hủy',
