  ...
  implements OnInit, AfterViewInit
{
  ...
  @ViewChild('tbHistoryFile', { static: false })
  tbHistoryFileRef!: ElementRef<HTMLDivElement>;
  formGroup: FormGroup;
  @Input() dataInput: any;
  ...
  private tryHydrateFiles() {
    ...
    if (
      !this.fileTable ||
      !Array.isArray(this._existingFilesBuf) ||
      this._existingFilesBuf.length === 0
    )
      return;
    ...
    const mapped = this._existingFilesBuf.map((f: any, idx: number) => {
      ...
      const fileName =
        f.FileName ?? f.fileName ?? f.SavedFileName ?? f.savedFileName ?? '';
      const serverPath =
        f.ServerPath ?? f.serverPath ?? f.FilePath ?? f.filePath ?? '';
      const originName =
        f.OriginName ??
        f.originName ??
        f.OriginalFileName ??
  ...
  removeFile(rowData: any) {
    ...
    const idx = this.fileList.findIndex(
      (x: any) => x.uid === uid || x === rowData.file
    );
    if (idx < 0) return;
  ...
  save() {
    ...
    const toNumber = (v: any) =>
      v == null || v === '' ? 0 : Number(v.toString().replace(/\D/g, '')) || 0;
    ...
    this.vehicleRepairHistoryService.saveData(payloadMaster).subscribe({
      next: (res) => {
        ...
        const historyID =
          res?.data?.ID ?? res?.data?.HistoryID ?? this.dataInput?.ID ?? 0;
        ...
        if (newFiles.length === 0) {
          ...
          if (deleted.length === 0) {
            this.notification.success(
              'Thành công',
              'Đã lưu thông tin lịch sử sửa chữa'
            );
            this.formSubmitted.emit();
            this.activeModal.close('save');
            return;
            ...
            .subscribe({
              next: (r2) => {
                if (r2?.status === 1) {
                  ...
                  this.notification.error(
                    NOTIFICATION_TITLE.error,
                    r2?.message || 'Cập nhật file thất bại'
                  );
                }
              },
              error: (e) =>
                this.notification.error(
                  NOTIFICATION_TITLE.error,
                  e?.error?.message || 'Cập nhật file thất bại'
                ),
            });
          return;
        }

        const filesToUpload: File[] = newFiles.map(
          (f: any) => f.originFile as File
        );
        ...
        this.vehicleRepairHistoryService
          ...
          .subscribe({
            next: (uploadRes) => {
              ...
              console.log(
                'uploadRes.keys =',
                uploadRes?.data?.map((x: any) => Object.keys(x))
              );
                ...
                .subscribe({
                  next: (filesRes) => {
                    if (filesRes?.status === 1) {
                      this.notification.success(
                        'Thành công',
                        `Đã lưu và upload ${filesMeta.length} file`
                      );
                      this.formSubmitted.emit();
                      this.activeModal.close('save');
                    } else {
                      this.notification.error(
                        NOTIFICATION_TITLE.error,
                        filesRes?.message || 'Lưu metadata file thất bại'
                      );
                    }
                  },
                  error: (err) =>
                    this.notification.error(
                      NOTIFICATION_TITLE.error,
                      err?.error?.message || 'Lưu metadata file thất bại'
                    ),
                });
            },
            error: (err) =>
