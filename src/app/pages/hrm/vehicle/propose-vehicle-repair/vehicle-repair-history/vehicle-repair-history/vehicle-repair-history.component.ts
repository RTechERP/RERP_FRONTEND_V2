...
export class VehicleRepairHistoryComponent implements AfterViewInit {
  ...
  @ViewChild('vehicleRepairHistoryTable', { static: false }) vehicleRepairHistoryTableEl!: ElementRef;
  searchText: string = '';
  ...
  drawTableFile() {
    ...
    if (!element) {
      console.warn('Element #vehicleRepairHistoryFileTable not found, retrying...');
  ...
  drawTableDetail() {
    ...
    if (this.vehicleRepairHistoryTable) {
          ...
          columns: [
            {
              ...
              columns: [
               ...
               // { title: 'Người duyệt', field: 'ApproveName', width: 190 },
                ...
                { title: 'Người đề xuất', field: 'EmployeePropose', width: 120 },
                { title: 'Nội dung đề xuất', field: 'ProposeContent', width: 120, formatter: 'textarea' },
                { title: 'Lý do', field: 'Reason', width: 120, formatter: 'textarea' },
            ...
            {
               cssClass: "group-ncc",
              title: 'THÔNG TIN NHÀ CUNG CẤP', columns: [
            ...
            { title: 'Ghi chú', field: 'Note', width: 250, formatter: 'textarea' },
          ]
        }
      );
      this.vehicleRepairHistoryTable.on('rowClick', (evt, row: RowComponent) => {
        const rowData = row.getData();
        const ID = rowData['ID'];
        this.vehicleRepairHistoryService.getVehicleRepairHistoryFile(ID).subscribe({
          next: (respon) => {
          ...
          error: (err) => {
            ...
            this.notification.error('Thông báo', 'Không thể tải danh sách file đính kèm');
          }
        });
      });
