...
export class InventoryDemoService {
  private url = `${API_ORIGIN}api/InventoryDemo/`;
...
getInventoryDemo(request: any) {
  ...
  getInventoryBorrowSupplier(request: any): Observable<any> {
    return this.http.post<any>(`${this.url + `get-inventory-borrow-ncc-Demo`}`, request);
  }
  getInventoryNCCAjax():string {
  return `${this.url}get-inventory-borrow-ncc-Demo`; 
}
getBorrowImportExportProductRTC(ProductID?: number, WarehouseID?: number): Observable<any> {
  ...
  return this.http.get<any>(`${this.url}get-borrow-import-export-product-rtc`, { params });
}
}
