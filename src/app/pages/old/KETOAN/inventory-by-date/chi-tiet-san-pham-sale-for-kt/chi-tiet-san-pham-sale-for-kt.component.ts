...
export class ChiTietSanPhamSaleForKtComponent implements OnInit, AfterViewInit{
  ...
  loadData(): void {
    this.inventoryByDateService.getImportExportInventoryByDate(this.productSaleId, this.warehouseCode, this.dateValues).subscribe((response: any) => {
      if (response.status === 1 && response.data) {
        this.dtImport = response.data.dtImport;
        this.dtExport = response.data.dtExport;
        this.dtHistory = response.data.dtHistory;
        this.tb_ImportTable.replaceData(this.dtImport);
        this.tb_ExportTable.replaceData(this.dtExport);
      }
    });
  }
  ...
  initImportTable(): void {
    this.tb_ImportTable = new Tabulator(this.importTableElementRef.nativeElement, {
      ...
      layout: 'fitColumns',
      columns: [
        { title: 'Số phiếu', field: 'BillImportCode' },
        { title: 'Ngày tạo', field: 'CreatDate' },
        { title: 'Nhà cung cấp', field: 'Suplier' },
        { title: 'Số lượng', field: 'Qty' },
        { title: 'Duyệt', field: 'Status' },
      ],
    });
  }
