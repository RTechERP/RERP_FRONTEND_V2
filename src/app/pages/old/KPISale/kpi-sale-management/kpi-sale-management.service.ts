...
export class KpiSaleManagementService {
  ...
  calculate(request: any): Observable<KpiApiResponse<any[]>> {
    return this.http.post<KpiApiResponse<any[]>>(`${this.apiUrl}/calculate`, request);
  }

  getResults(employeeId?: number, periodId?: number, templateId?: number): Observable<KpiApiResponse<any[]>> {
    let params = new HttpParams();
    if (employeeId) {
      params = params.set('employeeId', employeeId.toString());
    }
