...
export class BillDocumentExportComponent implements OnInit, AfterViewInit {
    ...
    this.billExportService.getBillDocumentExport(this.id).subscribe({
      next: (res) => {
        if (res?.data && Array.isArray(res.data)) {
          ...
          this.table_billDocumentExport?.replaceData(
            this.dataBillDocumentExport
          );
        }
      },
      ...
      next: (res) => {
        if (res?.data && Array.isArray(res.data)) {
          ...
          this.table_billDocumentExportLog?.replaceData(
            this.dataBillDocumentExportLog
          );
        }
      },
    ...
    for (const item of updatedData) {
      ...
      if (item.Status == null || item.Status === '' || item.Status == 0) {
        this.notification.warning(
          ...
          `Vui lòng chọn trạng thái chứng từ ${code}.`
        );
      ...
      if (item.Status === 2 && (!item.Note || item.Note.trim() === '')) {
        this.notification.warning(
          NOTIFICATION_TITLE.warning,
          `Vui lòng nhập lý do hủy cho chứng từ ${code}.`
        );
  ...
  drawTable() {
    ...
    if (!this.table_billDocumentExport) {
      this.table_billDocumentExport = new Tabulator(
        '#table_billDocumentExport',
        {
          index: 'ID',
    ...
    if (!this.table_billDocumentExportLog) {
      this.table_billDocumentExportLog = new Tabulator(
        '#table_billDocumentExportLog',
        {
          index: 'ID',
