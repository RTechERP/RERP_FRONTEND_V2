...
interface BillExport {
  ...
  IsApproved?: boolean;
  IsTransfer: boolean;
  TransferType?: number; // 1: Chuyển kho thường, 2: Chuyển kho nội bộ, 3: Chuyển sang kho Demo
  ProductGroupTransferID?: number; // Loại kho đích (KhoTypeID) cho chuyển kho nội bộ
  ProductGroupRTCTransferID?: number; // Loại kho đích (KhoTypeID) cho kho Demo
  BillImportTechID?: number; // Liên kết đến phiếu nhập kỹ thuật
...
export class BillExportDetailNewComponent
  ...
  dataCbbProductGroupTransfer: any[] = []; // Danh sách ProductGroup cho chuyển kho nội bộ
  dataCbbProductGroupDemo: any[] = []; // Danh sách ProductGroup cho kho Demo (không bao gồm IsDeleted)
  dataCbbSender: any[] = [];
  dataCbbSupplier: any[] = [];
  ...
  constructor(
    ...
    this.validateForm = this.fb.group({
      ...
      SupplierID: [0],
      IsTransfer: [false],
      TransferType: [1], // 1: Chuyển kho thường (mặc định)
      Reference: [''],
      WareHouseTranferID: [null],
      ProductGroupTransferID: [null], // Loại kho đích (KhoTypeID) cho chuyển kho nội bộ
      ProductGroupRTCTransferID: [null], // Loại kho đích (KhoTypeID) cho kho Demo
      BillImportTechID: [null], // Liên kết đến phiếu nhập kỹ thuật
      DeadlineDate: [{ value: null, disabled: true }],
      DeadlineTime: [{ value: null, disabled: true }],
    });

    // Setup initial date validators based on default status
    this.updateDateValidators(2);
  }
  ...
  private setupFormSubscriptions(): void {
    ...
    this.validateForm
      ...
      .subscribe((khoTypeID: number) => {
        ...
        // Cập nhật lại danh sách ProductGroup cho chuyển kho nội bộ
        this.getDataCbbProductGroupTransfer();
      });

    // Lắng nghe thay đổi loại chuyển kho
    this.validateForm
      .get('TransferType')
      ?.valueChanges.pipe(takeUntil(this.destroy$))
      .subscribe((transferType: number) => {
        this.onTransferTypeChange(transferType);
      });

    // Lắng nghe thay đổi checkbox chuyển kho
    this.validateForm
      .get('IsTransfer')
      ?.valueChanges.pipe(takeUntil(this.destroy$))
      .subscribe((isTransfer: boolean) => {
        if (!isTransfer) {
          // Reset các giá trị khi bỏ chọn chuyển kho
          this.validateForm.patchValue({
            TransferType: 0,
            WareHouseTranferID: null,
            ProductGroupTransferID: null,
            ProductGroupRTCTransferID: null,
          });
        } else {
          // Khi check "Chuyển kho", load dropdown tương ứng
          const transferType = this.validateForm.get('TransferType')?.value || 0;
          this.onTransferTypeChange(transferType);
        }
      });
  ...
  private handleWarehouseReleaseFlow(): void {
    ...
    // Enable Deadline khi Status = 6 (Yêu cầu xuất kho)
    if (this.newBillExport.Status === 6) {
      ...
      next: (res) => {
        if (res?.data) {
          ...
          this.validateForm.patchValue({
            ...
            ProductGroupTransferID: data.ProductGroupTransferID || null,
            ProductGroupRTCTransferID: data.ProductGroupRTCTransferID || null,
          });

          // Parse Deadline từ API thành DeadlineDate + DeadlineTime
          if (data.Status === 6) {
            this.validateForm.get('DeadlineDate')?.enable();
            this.validateForm.get('DeadlineTime')?.enable();
          }
          if (data.Deadline) {
            ...
            this.validateForm.patchValue({ DeadlineDate: dl });
            const hours = dl.getHours();
            if (hours >= 13) {
              this.validateForm.patchValue({ DeadlineTime: 2 }); // Chiều
            } else {
              this.validateForm.patchValue({ DeadlineTime: 1 }); // Sáng
            }
          }
        ...
        if (res?.data) {
          ...
          this.dataDetail = rawData.map((item: any) => {
            ...
            return {
              ...
              TemQty: item.TemQty || 0,
              IsTemVerify: item.IsTemVerify || false,
            };
          });
          this.refreshGrid();
  ...
  /** Giới hạn ngày Deadline: chỉ cho chọn từ ngày CreatDate trở đi (chỉ áp dụng khi Status = 6) */
  disabledDeadlineDate = (current: Date): boolean => {
    const status = this.validateForm.get('Status')?.value;
    if (status !== 6) return false; // Không giới hạn khi không phải Yêu cầu xuất kho

    const creatDate = this.validateForm.get('CreatDate')?.value;
    if (!creatDate || !current) return false;

    const creatDay = new Date(creatDate);
    creatDay.setHours(0, 0, 0, 0);
    const currentDay = new Date(current);
    currentDay.setHours(0, 0, 0, 0);
    return currentDay.getTime() < creatDay.getTime();
  };

  /** Validate khi chọn ngày Deadline (chỉ validate khi Status = 6) */
  onDeadlineDateChange(date: Date): void {
    if (!date) return;
    const status = this.validateForm.get('Status')?.value;
    if (status !== 6) return; // Không validate khi không phải Yêu cầu xuất kho

    const creatDate = this.validateForm.get('CreatDate')?.value;
    if (creatDate) {
      const creat = new Date(creatDate);
      creat.setHours(0, 0, 0, 0);
      const sel = new Date(date);
      sel.setHours(0, 0, 0, 0);
      if (sel.getTime() < creat.getTime()) {
        this.notification.warning(
          NOTIFICATION_TITLE.warning,
          'Deadline phải bắt đầu từ ngày yêu cầu xuất kho!'
        );
        this.validateForm.patchValue({ DeadlineDate: null });
      }
    }
  }

  /** Nối DeadlineDate + DeadlineTime thành chuỗi ISO local time để gửi về BE */
  private buildDeadlineValue(formValues: any): string | null {
    if (!formValues.DeadlineDate) return null;
    const d = new Date(formValues.DeadlineDate);
    let hours = 0;
    let minutes = 0;
    if (formValues.DeadlineTime === 1) {
      hours = 8;   // Sáng = 8:00
      minutes = 0;
    } else if (formValues.DeadlineTime === 2) {
      hours = 13;  // Chiều = 13:30
      minutes = 30;
    }
    // Tạo chuỗi ISO local time để tránh bị chuyển sang UTC khi JSON.stringify
  ...
  /** Chuyển Date thành chuỗi ISO local time để tránh bị chuyển sang UTC khi JSON.stringify */
  private toLocalISOString(date: any): string | null {
    if (!date) return null;
  ...
  /** Xử lý thay đổi loại chuyển kho */
  onTransferTypeChange(transferType: number): void {
    // Reset các giá trị đích tùy theo loại chuyển kho
    switch (transferType) {
      case 1: // Chuyển kho thường
        this.validateForm.patchValue({
          ProductGroupTransferID: null,
          ProductGroupRTCTransferID: null,
        });
        break;
      case 2: // Chuyển kho nội bộ
        this.validateForm.patchValue({
          WareHouseTranferID: null,
          ProductGroupRTCTransferID: null,
        });
        // Load danh sách ProductGroup cho chuyển kho nội bộ
        this.getDataCbbProductGroupTransfer();
        break;
      case 3: // Chuyển sang kho Demo
        this.validateForm.patchValue({
          WareHouseTranferID: null,
          ProductGroupTransferID: null,
        });
        // Load danh sách loại kho cho kho Demo (không bao gồm IsDeleted)
        this.getDataCbbProductGroupDemo();
        break;
    }
  }
  ...
  loadReferenceLinks(): void {
    ...
    const transferType = this.validateForm.get('TransferType')?.value || 1;

    if (billExportID <= 0 || !isTransfer) return;

    this.billExportService.getBillImportByBillExportID(billExportID, transferType).subscribe({
      next: (res: any) => {
        if (res?.status === 1 && res?.data) {
          const billImports = Array.isArray(res.data) ? res.data : [res.data];
          this.referenceLinks = billImports
            .filter((item: any) => item && item.ID > 0)
            .map((item: any) => {
              // Map field names: BillImport (type 1,2) vs BillImportTechnical (type 3)
              const code = transferType === 3 ? item.BillCode : item.BillImportCode;
              const khoTypeID = transferType === 3 ? (item.GroupTypeID || 0) : (item.KhoTypeID || 0);
              const warehouse = this.dataCbbWareHouseTransfer.find(
                (w: any) => w.ID === item.WarehouseID
              );
              return {
                id: item.ID,
                text: item.IsDeleted ? `${code} - đã xóa` : code,
                khoTypeID: khoTypeID,
  ...
  onReferenceLinkClick(link: any): void {
    ...
    if (link.transferType === 3) {
      // TransferType 3: Mở BillImportTechnicalFormComponent
      import('../../../bill-import-technical/bill-import-technical-form/bill-import-technical-form.component').then(m => {
        const modalRef = this.modalService.open(m.BillImportTechnicalFormComponent, {
          backdrop: 'static',
          keyboard: false,
          fullscreen: true,
        });
        modalRef.componentInstance.masterId = link.id;
      ...
      // TransferType 1, 2: Mở BillImportDetailNewComponent
  ...
  /** Load danh sách ProductGroup cho chuyển kho nội bộ (loại bỏ ProductGroup hiện tại) */
  getDataCbbProductGroupTransfer(): void {
    this.billExportService.getCbbProductGroup().subscribe({
      next: (res: any) => {
        const list = Array.isArray(res?.data) ? res.data : [];
        const currentKhoTypeID = this.validateForm.get('KhoTypeID')?.value;

        // Lọc bỏ ProductGroup hiện tại để chỉ hiển thị các ProductGroup khác
        this.dataCbbProductGroupTransfer = list.filter(
          (item: any) => item.ID !== currentKhoTypeID
        );
      },
      error: (err: any) => {
        this.notification.error(NOTIFICATION_TITLE.error, err?.error?.message || `${err.error}\n${err.message}`,
          { nzStyle: { whiteSpace: 'pre-line' } });
        this.dataCbbProductGroupTransfer = [];
      },
    });
  }

  /** Load danh sách ProductGroup cho kho Demo (không lấy IsDeleted, loại bỏ ProductGroup hiện tại) */
  getDataCbbProductGroupDemo(): void {
    // this.billExportService.getCbbProductGroupRTCTransfer(1).subscribe({
    //   next: (res: any) => {
    //     const list = Array.isArray(res?.data) ? res.data : [];

    //     this.dataCbbProductGroupDemo = list;
    //   },
    //   error: (err: any) => {
    //     this.notification.error(NOTIFICATION_TITLE.error, err?.error?.message || `${err.error}\n${err.message}`,
    //       { nzStyle: { whiteSpace: 'pre-line' } });
    //     this.dataCbbProductGroupDemo = [];
    //   },
    // });
  }

  /** Load danh sách kho đích cho chuyển kho thường (Type 1) */
  // getDataCbbWareHouseTransfer(): void {
  //   // Sử dụng API getCbbProductGroup để lấy danh sách warehouse
  //   // Hoặc nếu có API riêng cho warehouse, thay đổi tại đây
  //   this.billExportService.getCbbProductGroup().subscribe({
  //     next: (res: any) => {
  //       this.dataCbbWareHouseTransfer = Array.isArray(res?.data) ? res.data : [];
  //     },
  //     error: (err: any) => {
  //       console.error('Error getting warehouses for transfer:', err);
  //       this.dataCbbWareHouseTransfer = [];
  //     },
  //   });
  // }
  ...
  private mapTableDataToBillExportDetails(tableData: any[]): any[] {
    return tableData.map((row: any, index: number) => {
      ...
      return {
        ...
        ExpectReturnDate: this.toLocalISOString(row.ExpectReturnDate),
        ...
        IsHeavy: row.IsHeavy || false,
        IsTemVerify: row.IsTemVerify || false,
        TemQty: row.TemQty || 0,
      };
    });
  }
    ...
    try {
      ...
      const payload = {
        BillExport: {
          ...
          ProductGroupTransferID: formValues.ProductGroupTransferID,
          ProductGroupRTCTransferID: formValues.ProductGroupRTCTransferID,
          BillImportTechID: formValues.BillImportTechID || this.newBillExport.BillImportTechID,
          IsPrepared: false,
          IsReceived: false,
          IsDeleted: false,
          Deadline: this.buildDeadlineValue(formValues),
        },
      ...
      next: (res: any) => {
        if (res.status === 1 && res.data && res.data.length > 0) {
          ...
          files.forEach((file: any, index: number) => {
            // Xử lý serverPath: bỏ phần IP, chỉ giữ lại phần path
            let filePath = file.ServerPath || file.serverPath || '';

            // Loại bỏ phần IP (format: \\IP\path hoặc //IP/path)
            // Giữ lại phần sau IP: \path hoặc /path
            if (filePath) {
              // Replace \\ hoặc // ở đầu
              filePath = filePath.replace(/^[\\\/]+/, '');
              // Tìm vị trí của \ hoặc / thứ 2 (sau IP)
              const secondSlashIndex = filePath.search(/[\\\/]/);
              if (secondSlashIndex > 0) {
                // Lấy phần sau IP
                filePath = filePath.substring(secondSlashIndex + 1);
              }
              // Chuẩn hóa path: thay \ thành /
              filePath = filePath.replace(/\\/g, '/');
            }

            this.downloadSinglePOFileByPath(filePath, file.FileName || file.fileName, index === files.length - 1);
          });
        } else {
          this.notification.warning(
            NOTIFICATION_TITLE.warning,
  ...
  /**
   * Download a single file by server path using /api/share/ endpoint
   * @param filePath - The file path (after removing IP)
   * @param fileName - The file name to download
   * @param isLast - Whether this is the last file (to hide loading)
   */
  private downloadSinglePOFileByPath(filePath: string, fileName: string, isLast: boolean): void {
    if (!filePath) {
    ...
    // Tạo URL download qua /api/share/
    const downloadUrl = `/api/share/${encodeURIComponent(filePath)}`;
