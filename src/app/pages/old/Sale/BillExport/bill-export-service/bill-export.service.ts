...
export class BillExportService {
  ...
  getCbbProductGroup(): Observable<any> {
    return this.http.get<any>(
      environment.host + `api/ProductGroup`
    );
  }

  // Get Product Group RTC Transfer (không lấy IsDeleted) - Dùng cho chuyển kho Demo
  getCbbProductGroupRTCTransfer(warehouseType: number): Observable<any> {
    return this.http.get<any>(
      environment.host + `api/ProductGroupRTC/get-all?warehouseType=${warehouseType}`
    );
  }
