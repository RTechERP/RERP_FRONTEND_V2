...
export class ScanBillImportComponent implements OnInit, AfterViewInit {
  ...
  loadBillImportQR() {
    ...
    this.billImportService
      .getBillImportQR(1, this.searchParams.keyword)
      .subscribe({
        next: (res) => {
          if (res?.data && Array.isArray(res.data)) {
            ...
            if (this.table_BillImport) {
              const existingIDs = this.table_BillImport
                .getData()
                .map((row: any) => row.ID);
    ...
    for (const row of allRows) {
      try {
        ...
        const fullResponse = await firstValueFrom(
          this.billImportService.getBillImportByID(row.ID)
        );
