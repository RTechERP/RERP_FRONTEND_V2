...
@Component({
  selector: 'app-bill-import-prime',
  imports: [
    CommonModule,
    FormsModule,
    ReactiveFormsModule,
  ...
  templateUrl: './bill-import-prime.component.html',
  styleUrl: './bill-import-prime.component.css'
})
export class BillImportPrimeComponent {
exportProgress = { current: 0, total: 0, fileName: '' };
  ...
  onRowClick(rowData: any): void {
    const newId = rowData?.ID || 0;
    this.selectedRow = rowData;
    this.updateTabDetailTitle();
    if (newId === this.id) {
      // Cùng ID, không cần gọi lại API
      return;
    }
    this.id = newId;
    if (this.id > 0) {
  ...
  openModalBillImportDetail(ischeckmode: boolean): void {
    ...
    modalRef.result.then((result) => {
      // Chỉ reload khi modal trả về true (đã lưu thành công)
      if (result === true) {
        this.loadDataBillImport();
        if (this.selectedRow?.ID > 0) {
          // Reset id guard để force reload detail sau khi save
          const savedId = this.selectedRow.ID;
          this.id = 0;
          this.id = savedId;
          this.getBillImportDetail(savedId);
        }
      }
    }).catch(() => {
      // Người dùng đóng modal mà không lưu → không làm gì
    });
  }
