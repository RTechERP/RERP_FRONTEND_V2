...
export class ListProductProjectComponent
    ...
    this.columnDefinitions = [
      ...
      },
      {
        ...
        formatter: (_r: any, _c: any, v: any) => {
          if (!v) return '';
          const dates = String(v).split(/\r?\n|,/).map((d: string) => d.trim()).filter(Boolean);
          const shown = dates.slice(0, 3);
          const content = shown.join('<br>') + (dates.length > 3 ? '<br>...' : '');
          return `<div style="white-space:normal; line-height:1.4; overflow:hidden; display:-webkit-box; -webkit-line-clamp:3; -webkit-box-orient:vertical;">${content}</div>`;
        },
        exportCustomFormatter: (_r: any, _c: any, v: any) => {
          if (!v) return '';
          const dates = String(v).split(/\r?\n|,/).map((d: string) => d.trim()).filter(Boolean);
          return dates.slice(0, 3).join('\n') + (dates.length > 3 ? '\n...' : '');
        },
        excelExportOptions: { width: 36 },
      },
      {
        id: 'ExportDates',
        field: 'ExportDates',
