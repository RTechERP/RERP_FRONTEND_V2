...
export class TradePriceComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    this.loadEmployee(), this.loadProject(), this.loadCustomer();
  }
  ...
  loadTradePrice(
    ...
    this.tradePriceService
      .getTradePrice(employeeId, saleAdminId, projectId, customerId, keyword)
      .subscribe({
        next: (response) => {
          if (response.status === 1) {
    ...
    this.tradePriceService.getTradePriceDetail(this.selectedId).subscribe({
      next: (response) => {
        if (response.status === 1) {
          ...
          const MainData = this.mainData.find(
  ...
  onSaleApprovedAndRequest(isApprove: number) {
    if (this.selectedRows.length < 1) {
      this.notification.error(
        'Thông báo!',
        'Vui lòng chọn ít nhất 1 dự án cần thay đổi trạng thái!'
      );
      return;
    }

    const actionText =
      isApprove === 1 ? 'CHỐT' : isApprove === 2 ? 'HỦY CHỐT' : 'YÊU CẦU DUYỆT';
          ...
          if (isApprove === 3) {
            if (row.IsApprovedSale === 3) {
              return of({
                row,
                status: 'skipped',
                reason: 'Dự án đã được yêu cầu duyệt',
              });
            }
            if (row.IsApprovedSale === 0 || row.IsApprovedSale === 2) {
              return of({
                row,
                status: 'skipped',
                reason: 'Dự án chưa được chốt nên không thể yêu cầu duyệt',
              });
            }
          }
          if (
            row.IsApprovedSale === 3 &&
            (isApprove === 2 || isApprove === 1)
          ) {
            return of({
              row,
              status: 'skipped',
              reason:
                'Dự án đã được yêu cầu duyệt, không thể thay đổi trạng thái',
            });
          }
        ...
        forkJoin(requests).subscribe((results: any[]) => {
          ...
          if (skipped.length > 0) {
            const skippedDetails = skipped
              .map(
                (r: any) => `${r.row?.ProjectCode || r.row?.ID}: ${r.reason}`
              )
              .join('; ');
            this.notification.error(
          ...
          if (failed.length > 0) {
            const failedDetails = failed
              .map(
                (r: any) =>
                  `${r.row?.ProjectCode || r.row?.ID}: ${
                    r.message || 'Không thể cập nhật dữ liệu!'
                  }`
              )
              .join('; ');
            this.notification.error(
  ...
  onLeaderApproved(isApprove: number) {
    ...
    this.modal.confirm({
      ...
      nzOnOk: () => {
        const requests = this.selectedRows.map((row: any) => {
          if (
            (isApprove === 1 || isApprove === 2) &&
            row.IsApprovedLeader === 3 &&
            (row.IsApprovedBGD === 0 || row.IsApprovedBGD === 1)
          ) {
            return of({
              row,
              status: 'skipped',
              reason: 'Dự án đang chờ BGĐ duyệt, không thể thay đổi trạng thái',
            });
          }
          if (isApprove === 1 && row.IsApprovedLeader === 1) {
            return of({
              row,
              status: 'skipped',
              reason: 'Dự án đã được duyệt rồi',
            });
          }
          if (isApprove === 2 && row.IsApprovedLeader === 2) {
            return of({
              row,
              status: 'skipped',
              reason: 'Dự án đã được hủy duyệt rồi',
            });
          }
          if (isApprove === 3) {
          ...
          const DATA = {
            ID: row.ID,
            IsApprovedLeader: isApprove,
            IsApprovedBGD: valueIsApprovedBGD,
          };
    ...
    this.modal.confirm({
      ...
      nzOnOk: () => {
        const requests = this.selectedRows.map((row: any) => {
          ...
          const DATA = {
            ID: row.ID,
            IsApprovedBGD: isApprove,
            IsApprovedLeader: valueIsApprovedLeader,
          };
          ...
          if (
            ...
            DATA.IsApprovedLeader = isApprove;
          }
  ...
  initMainTable(): void {
    this.tb_MainTable = new Tabulator(this.tb_MainTableElement.nativeElement, {
      ...
      columns: [
        ...
        {
          title: 'Ngày sale cập nhật',
          field: 'SaleApprovedDate',
          sorter: 'string',
          width: 150,
        },
        {
          title: 'Ngày leader duyệt',
          field: 'LeaderApprovedDate',
          sorter: 'string',
          width: 150,
        },
        {
          title: 'Ngày BGD duyệt',
          field: 'BGDApprovedDate',
          sorter: 'string',
          width: 150,
        },
        {
    ...
    this.tb_MainTable.on('rowClick', (e: any, row: RowComponent) => {
      ...
      setTimeout(() => {
        if (this.tb_Detail) {
          ...
          if (columns[3]) {
            columns[0].title = `Tổng CM/Set = ${this.formatMoney(
              this.selectedRow?.TotalCMPerSET
            )}`;
            columns[1].title = `EXW = ${this.formatMoney(
              this.selectedRow?.EXW
            )}`;
            columns[2].title = `Margin = ${this.formatMoney(
              this.selectedRow?.Margin
            )}`;
            columns[3].title = `Lợi nhuận = ${this.formatMoney(
              this.selectedRow?.TotalProfit2
            )}`;
            columns[4].title = `Tỷ lệ lợi nhuận = ${this.formatMoney(
              this.selectedRow?.TotalProfitPercent
            )}`;
