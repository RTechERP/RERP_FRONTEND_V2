...
export class BillImportTechnicalComponent implements OnInit, AfterViewInit {
    ...
    onApprove() {
        ...
        const confirmMsg =
            billCount === 1
                ? `Bạn có chắc chắn muốn duyệt phiếu "${selectedBills[0].BillCode || selectedBills[0].ID
                }" không?`
    ...
    onUnApprove() {
        ...
        const confirmMsg =
            billCount === 1
                ? `Bạn có chắc chắn muốn HỦY DUYỆT phiếu "${selectedBills[0].BillCode || selectedBills[0].ID
                }" không?`
