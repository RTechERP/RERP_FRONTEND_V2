...
export class EmployeeComponent implements OnInit, AfterViewInit {
    ...
    if (!document.getElementById('tb_employee') || this.tabulatorEmployee) {
      return; // Đã khởi tạo hoặc chưa có DOM
        ...
        this.tabulatorEmployee = new Tabulator('#tb_employee', {
          ...
          // // ajaxURL: this.employeeService.getEmployeesURL(),
          // ajaxConfig: 'GET',
          ...
          //       searchData.keyword || ''
          //     )
          //   return this.employeeService.filterEmployee(searchData.status || 0, searchData.department || 0, searchData.keyword || '').toPromise();
          // },
  ...
  <button style="font-size: 0.75rem; border: 1px solid black;">Đang làm việc</button>
  <button style="font-size: 0.75rem; border: 1px solid black; background-color: wheat;">Nghỉ việc</button>
  <button style="font-size: 0.75rem; border: 1px solid black; background-color: yellow;">Sắp hết HĐ(1T)</button>
  <button style="font-size: 0.75rem; border: 1px solid black; background-color: red; color: white;">Sắp hết HĐ(15N)</button>
</div>
  `,
              frozen: true,
              columns: [
  ...
  openEditModal() {
    ...
    this.employeeForm.patchValue({
      ...
      DateStartContractThreeYear:
        this.selectedEmployee.DateStartContractThreeYear,
  ...
  async exportToExcel() {
    ...
    const exportData = tableData
      ...
      .map((employee, idx) => {
        ...
        return {
          ...
          'Tình trạng hôn nhân': safe(
            employee.TinhTrangHonNhanID === 0 ? 'Đã kết hôn' : 'Độc thân'
          ),
          ...
          'Tình trạng': safe(
            employee.Status === 0 ? 'Đang làm việc' : 'Nghỉ việc'
          ),
          // Checklist hồ sơ
    ...
    worksheet.columns = [
      ...
      {
        header: 'Quận/Huyện thường trú',
        key: 'Quận/Huyện thường trú',
        width: 20,
      },
      {
        header: 'Phường/Xã thường trú',
        key: 'Phường/Xã thường trú',
        width: 20,
      },
      ...
      {
        header: 'Ngày bắt đầu đóng BHXH',
        key: 'Ngày bắt đầu đóng BHXH',
        width: 20,
      },
      {
        header: 'Mức đóng BHXH hiện tại',
        key: 'Mức đóng BHXH hiện tại',
        width: 20,
      },
      ...
      {
        header: 'CMND/CCCD (Checklist)',
        key: 'CMND/CCCD (Checklist)',
        width: 15,
      },
      ...
      {
        header: 'Đánh giá chuyển hợp đồng',
        key: 'Đánh giá chuyển hợp đồng',
        width: 15,
      },
