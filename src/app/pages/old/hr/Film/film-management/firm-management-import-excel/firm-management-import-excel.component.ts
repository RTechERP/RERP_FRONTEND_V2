...
@Component({
  ...
  selector: 'app-firm-management-import-excel',
  templateUrl: './firm-management-import-excel.component.html',
  styleUrls: ['./firm-management-import-excel.component.css']
})
export class FirmManagementImportExcelComponent implements OnInit {
 @Input() table: any;
...
ngOnInit() {
    ...
    try {
      ...
      worksheet.eachRow((row, rowNumber) => {
        ...
        const rowData = {
          ...
          Code: getValue(2),
          Name: getValue(3),
          RequestResult: getValue(4),
          UnitName: getValue(5),
          PerformanceAVG: getValue(6),
        };
        data.push(rowData);
        validRecords++;
      });
  ...
  async saveExcelData() {
    ...
    // try {
    //   const res: any = await firstValueFrom(this.filmManagementService.getFilm({
    //     keyWord: ' ',
    //     checkAll: 1,
    //     productGroupID: 0,
    //     warehouseID: 0,
    ...
    const saveOneByOne = (index: number) => {
      ...
      setTimeout(() => {
        this.filmManagementService.saveData(productData).subscribe({
          next: (res: any) => {
            if (res.status === 1) {
              successCount++;
            } else {
  ...
  private loadUnit() {
    ...
    // this.filmManagementService.getFirm(1).subscribe((response: any) => {
    //   this.firmData = response.data;
    //   console.log("Firm:", this.firmData);
    // });
    // this.filmManagementService.getLocation(1).subscribe((response: any) => {
    //   this.locationData = response.data.location;
    //   console.log("Location", this.locationData);
    // });
    // this.filmManagementService.getProductRTCGroup().subscribe((resppon: any) => {
