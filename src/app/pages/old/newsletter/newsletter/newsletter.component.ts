...
export class NewsletterComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    menuBars: MenuItem[] = [
        ...
        {
            label: 'Công bố',
            icon: 'fas fa-check-circle fa-lg text-success',
            command: () => {
                this.onPublish(true);
            }
        },
    ...
    onPublish(isPublish: boolean = true) {
        if (!this.nightShiftTable) {
            this.notification.warning(NOTIFICATION_TITLE.warning, 'Bảng dữ liệu chưa được khởi tạo');
            return;
        }

        const isPublishText = isPublish ? 'công bố' : 'hủy công bố';
        const selectedRows = this.nightShiftTable.getSelectedRows();
        if (selectedRows.length === 0) {
            this.notification.warning(NOTIFICATION_TITLE.warning, `Vui lòng chọn bản tin cần ${isPublishText}`);
            return;
        }

        // Prepare confirmation message
        const confirmMessage = selectedRows.length === 1
            ? `Bạn có chắc chắn muốn ${isPublishText} bản tin "${selectedRows[0].getData()['Title']}"?`
            : `Bạn có chắc chắn muốn ${isPublishText} ${selectedRows.length} bản tin đã chọn?`;

        this.modal.confirm({
            nzTitle: `Xác nhận ${isPublishText}`.toUpperCase(),
            nzContent: confirmMessage,
            nzOkText: isPublishText.toUpperCase(),
            nzCancelText: 'HỦY',
            nzOkDanger: !isPublish,
            nzOkType: 'primary',
            nzOnOk: () => {
                // Delete all selected newsletters
                for (let row of selectedRows) {
