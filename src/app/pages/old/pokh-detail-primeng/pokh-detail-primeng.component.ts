...
import { CustomTreeTablePokh } from '../../../shared/custom-tree-table-pokh/custom-tree-table-pokh';
import { TreeColumnDef } from '../../../shared/custom-tree-table-pokh/tree-column-def.model';
import { TreeNode } from 'primeng/api';
import flatpickr from 'flatpickr';
import { Vietnamese } from 'flatpickr/dist/l10n/vn.js';
@Component({
  selector: 'app-pokh-primeng',
  imports: [
    NzCardModule,
    FormsModule,
    ReactiveFormsModule,
  ...
  templateUrl: './pokh-detail-primeng.component.html',
  styleUrl: './pokh-detail-primeng.component.css',
})
export class PokhDetailPrimengComponent implements OnInit, AfterViewInit {
  ...
  pokhForm!: NgForm;
  @ViewChild('productTreeTable', { static: false })
  productTreeTable!: CustomTreeTablePokh;
  ...
  // PrimeNG TreeTable Data cho sản phẩm
  productTreeData: TreeNode[] = [];
  productColumns: TreeColumnDef[] = [];
  ...
  //#region : PrimeNG TreeTable - Product Columns & Data Conversion

  /**
   * Format money value for display
   */
  private formatMoneyValue(value: number | null | undefined): string {
    if (value == null || isNaN(Number(value))) return '';
    return new Intl.NumberFormat('vi-VN', { maximumFractionDigits: 0 }).format(Number(value));
  }

  /**
   * Initialize product columns for PrimeNG TreeTable
   */
    ...
    this.productColumns = [
      ...
      },
      {
        ...
        editLookupConfig: {
          ...
          columns: [
            { field: 'ProductCode', header: 'Mã SP', width: '120px' },
            { field: 'ProductName', header: 'Tên sản phẩm', width: '250px' },
            { field: 'ProductNewCode', header: 'Mã nội bộ', width: '120px' },
            { field: 'Unit', header: 'ĐVT', width: '80px' },
            { field: 'ProductGroupName', header: 'Tên nhóm', width: '200px' },
          ],
          valueField: 'ID',
          displayField: 'ProductNewCode',
        },
        format: (value: any, rowData: any) => {
          if (!value) return 'Chọn SP';
          const product = this.dataProducts.find(p => p.ID === value);
          return product?.ProductNewCode || 'Chọn SP';
        },
      },
      {
        field: 'ProductCode',
      ...
      },
      {
        ...
        filterType: 'numeric',
        footerType: 'sum',
        footerFormat: { maximumFractionDigits: 0 },
        cssClass: 'text-right',
      },
      {
        field: 'FilmSize',
        header: 'Kích thước phim cắt/Model',
        width: '200px',
        editable: true,
        editType: 'text',
      },
      ...
      },
      {
        ...
        editMin: 0,
        filterType: 'numeric',
        format: (value: any) => this.formatMoneyValue(value),
        cssClass: 'text-right',
      },
      {
        field: 'UnitPrice',
      ...
      },
      {
        ...
        header: 'Tiền sau chiết khấu (chưa VAT)',
        width: '150px',
        filterType: 'numeric',
        footerType: 'sum',
        footerFormat: { maximumFractionDigits: 0 },
      ...
      },
      {
        ...
        editMin: 0,
        filterType: 'numeric',
        format: (value: any) => value != null && !isNaN(Number(value)) ? value + '%' : '',
        cssClass: 'text-right',
      },
      {
        field: 'TotalPriceIncludeVAT',
        header: 'Tổng tiền sau VAT',
        width: '150px',
        filterType: 'numeric',
        footerType: 'sum',
        footerFormat: { maximumFractionDigits: 0 },
      ...
      {
        field: 'PayDate',
        header: 'Ngày yêu cầu thanh toán',
        width: '150px',
        editable: true,
        editType: 'date',
        editDateFormat: 'dd/mm/yy',
  ...
  /**
   * Convert flat data with _children to TreeNode[]
   */
  convertToTreeNode(flatData: any[]): TreeNode[] {
    const treeData: TreeNode[] = [];
    const map = new Map<number, TreeNode>();
    const pending = new Map<number, any[]>();

    // 1. Automatically flatten nested tree data with _children if present
    const flattenList: any[] = [];
    const flatten = (items: any[]) => {
      items.forEach((item) => {
        const { _children, ...cleanItem } = item;
        flattenList.push(cleanItem);
    ...
    // 2. Create nodes - use deep clone to avoid reference sharing issues
    flattenList.forEach((item) => {
      const node: TreeNode = {
        key: item.ID,
        data: JSON.parse(JSON.stringify(item)), // Deep clone to prevent shared references
        children: [],
        expanded: true,
      };
    ...
    flattenList.forEach((item) => {
      ...
      if (item.ParentID && map.has(item.ParentID)) {
        ...
        // Parent not yet available - add to pending
    ...
    // 4. Process pending children
    pending.forEach((children, parentId) => {
      if (map.has(parentId)) {
        map.get(parentId)!.children!.push(...children);
      }
    });
  ...
  /**
   * Convert TreeNode[] back to flat data with _children
   */
  convertFromTreeNode(treeData: TreeNode[]): any[] {
    const result: any[] = [];

    const flatten = (nodes: TreeNode[]) => {
      nodes.forEach((node) => {
        const { children, parent, ...rest } = node;
        // Recursively flatten children first to get clean data
        const flattenedChildren = children && children.length > 0
          ? this.convertFromTreeNode(children)
          : [];
        // Extract row data - node.data contains the actual row
        let rowData: any;
        if (rest.data) {
          // Deep clone and remove parent from node.data
          const { parent: dataParent, ...dataRest } = rest.data;
          rowData = { ...dataRest };
        } else {
          rowData = { ...rest };
        }
        // Add flattened children (not TreeNode objects, just plain data)
        rowData._children = flattenedChildren;
        // Remove parent references if they exist
        delete rowData.parent;
        delete rowData._parent;
        result.push(rowData);
      });
    };
   ...
   */
  private recalculateRow(rowData: any): void {
    ...
    rowData.IntoMoney = unitPrice * qty;
    rowData.IntoMoneyAfterDiscount = rowData.IntoMoney - discountAmount;
    rowData.TotalPriceIncludeVAT = rowData.IntoMoneyAfterDiscount * (1 + vat / 100);
  }

  /**
   * Recalculate all totals from tree data
   */
  recalculateTotals(): void {
    ...
    const calculateFromTree = (nodes: TreeNode[]) => {
      nodes.forEach((node) => {
        const data = node.data;
        if (data.IntoMoney) totalBeforeVAT += Number(data.IntoMoney) || 0;
        if (data.TotalPriceIncludeVAT) totalAfterVAT += Number(data.TotalPriceIncludeVAT) || 0;
        if (data.DiscountAmount) totalDiscount += Number(data.DiscountAmount) || 0;

        if (node.children && node.children.length > 0) {
          calculateFromTree(node.children);
        }
      });
    };

    calculateFromTree(this.productTreeData);

    this.poFormData.totalPOBeforeVAT = totalBeforeVAT;
    this.poFormData.totalPO = totalAfterVAT;
    this.poFormData.moneyDiscount = totalDiscount;

    // Update responsible users money
    this.updateResponsibleUsersMoney();
    this.calculateTotalFromMoneyDiscount();
  }

  /**
   * Collect all IDs (including children) from a row
   */
  private collectChildIds(rowData: any): number[] {
    const ids: number[] = [rowData.ID];
    if (rowData._children && rowData._children.length > 0) {
      rowData._children.forEach((child: any) => {
        ids.push(...this.collectChildIds(child));
      });
    }
    return ids;
  }
  ...
  ngAfterViewInit(): void {
    // Initialize PrimeNG TreeTable columns
    ...
    if (this.isEditMode && this.selectedId > 0 && !this.isLoadingData) {
      ...
      // Initialize PrimeNG TreeTable với data rỗng
    ...
    this.POKHService.loadProducts().subscribe({
      next: (response: any) => {
        if (response.status === 1) {
          ...
          // Update product lookup config in PrimeNG TreeTable columns
          if (this.productColumns && this.productColumns.length > 0) {
            const productIdCol = this.productColumns.find(col => col.field === 'ProductID');
            if (productIdCol && productIdCol.editLookupConfig) {
              productIdCol.editLookupConfig.data = this.dataProducts;
            }
          }
        } else {
          this.notification.create(
        ...
        if (response.status === 1) {
          ...
          forkJoin([POKHProducts$, POKHFiles$, detailUser$]).subscribe(
            ([productsData, filesData, userDetailsData]) => {
              ...
              // Convert to PrimeNG TreeNode[] format
              this.productTreeData = this.convertToTreeNode(this.dataPOKHProduct);
              ...
              // PrimeNG TreeTable data is set via productTreeData binding
              // No need to call initProductDetailTreeList() anymore
  ...
  savePOKH() {
    ...
    // Get details from PrimeNG TreeTable (convert tree to flat)
    const details = this.convertFromTreeNode(this.productTreeData);
    const detailUsers =
  ...
  getTreeRows(data: any[]): any[] {
    ...
    data.forEach((row) => {
      // Remove parent/_parent to avoid circular reference before processing
      const { parent, _parent, ...cleanRow } = row;
      const processedRow = {
        ...cleanRow,
        KHID: !cleanRow.KHID || Object.keys(cleanRow.KHID).length === 0 ? 0 : cleanRow.KHID,
        IndexPO:
        ...
        DiscountAmount: row.DiscountAmount === '' || row.DiscountAmount === null || row.DiscountAmount === undefined ? null : Number(row.DiscountAmount),
        IntoMoneyAfterDiscount: row.IntoMoneyAfterDiscount === '' || row.IntoMoneyAfterDiscount === null || row.IntoMoneyAfterDiscount === undefined ? null : Number(row.IntoMoneyAfterDiscount),
  ...
  //#region : PrimeNG TreeTable Event Handlers

  /**
   * Handle cell value change from PrimeNG TreeTable
   */
  onProductCellValueChange(event: { rowKey: any; rowData: any; field: string; value: any }): void {
    if (this.lockEvents) return;

    const { rowData, field, value } = event;
    const quantity = Number(rowData.Qty) || 0;
    const unitPrice = Number(rowData.UnitPrice) || 0;
    const vat = Number(rowData.VAT) || 0;
    const discountAmount = Number(rowData.DiscountAmount) || 0;
    const debt = Number(rowData.Debt) || 0;
    ...
    rowData.IntoMoney = intoMoney;
    rowData.IntoMoneyAfterDiscount = intoMoneyAfterDiscount;
    rowData.TotalPriceIncludeVAT = totalWithVAT;

    // Calculate PayDate from BillDate + Debt days
    if (rowData.BillDate) {
      const payDate = new Date(rowData.BillDate);
      payDate.setDate(payDate.getDate() + debt);
      const year = payDate.getFullYear();
      const month = String(payDate.getMonth() + 1).padStart(2, '0');
      const day = String(payDate.getDate()).padStart(2, '0');
      rowData.PayDate = `${year}-${month}-${day}T00:00:00`;
    }

    // Update VAT for all rows if VAT is changed and current VAT is 0
    if (field === 'VAT' && value > 0) {
      // Find rows with VAT = 0 and empty VAT text, apply the same VAT
      const applyVATToEmptyRows = (nodes: TreeNode[]) => {
        nodes.forEach(node => {
          const data = node.data;
          if (data.VAT === 0 || data.VAT === null || data.VAT === undefined) {
            data.VAT = value;
            this.recalculateRow(data);
          }
          if (node.children && node.children.length > 0) {
            applyVATToEmptyRows(node.children);
          }
        });
      };
      applyVATToEmptyRows(this.productTreeData);
    }
  ...
  /**
   * Handle cell action (e.g., delete button click)
   */
  onProductCellAction(event: { field: string; rowData: any }): void {
    const { rowData } = event;
    if (event.field === 'actions') {
      this.deleteProductRow(rowData);
    }
  }

  /**
   * Handle product lookup selection
   */
  onProductLookupSelect(event: { selectedRow: any; field: string; rowData: any }): void {
    const { selectedRow, rowData } = event;

    if (selectedRow) {
      rowData.ProductID = selectedRow.ID;
      rowData.ProductNewCode = selectedRow.ProductNewCode;
      rowData.ProductCode = selectedRow.ProductCode;
      rowData.ProductName = selectedRow.ProductName;
      rowData.Unit = selectedRow.Unit;
      rowData.Maker = selectedRow.Maker;
      rowData.ProductGroupName = selectedRow.ProductGroupName;
  ...
  /**
   * Handle row click from PrimeNG TreeTable
   */
  onProductRowClick(rowData: any): void {
    this.selectedRow = rowData;
    console.log('selectedRow', this.selectedRow);
  }

  /**
   * Delete a product row (including children)
   */
  deleteProductRow(rowData: any): void {
    this.modal.confirm({
      ...
      nzOnOk: () => {
        ...
        const idsToDelete = this.collectChildIds(rowData);
        this.deletedPOKHDetailIds = Array.from(
          new Set([...this.deletedPOKHDetailIds, ...idsToDelete.filter(id => id > 0)])
        );

        // Remove from tree
        this.productTreeData = this.removeNodeFromTree(this.productTreeData, rowData.ID);
        this.dataPOKHProduct = this.convertFromTreeNode(this.productTreeData);
        this.recalculateTotals();
      },
    });
  }
  ...
  /**
   * Remove a node from tree by ID
   */
  private removeNodeFromTree(nodes: TreeNode[], id: number): TreeNode[] {
    ...
    nodes.forEach(node => {
      if (Number(node.key) !== id) {
        const newNode: TreeNode = {
          key: node.key,
          data: node.data,
          children: node.children ? this.removeNodeFromTree(node.children, id) : [],
  ...
  calculateTotalIterative(): void {
    ...
    allRows.forEach((row: RowComponent) => {
      ...
      while (stack.length > 0) {
        ...
        if (
          currentNode['DiscountAmount'] !== undefined &&
          !isNaN(currentNode['DiscountAmount'])
        ) {
          totalMoneyDiscount += Number(currentNode['DiscountAmount']);
        }
  ...
  addNewRow(): void {
    ...
    // Count total rows for STT
    const countRows = (nodes: TreeNode[]): number => {
      let count = nodes.length;
      nodes.forEach(n => {
        if (n.children && n.children.length > 0) {
          count += countRows(n.children);
        }
      });
      return count;
    };
    const totalRows = countRows(this.productTreeData);
    ...
    const newNode: TreeNode = {
      key: String(this.nextRowId),
      data: newRowData,
      children: [],
      expanded: true,
    };

    this.productTreeData = [...this.productTreeData, newNode];
    this.dataPOKHProduct = this.convertFromTreeNode(this.productTreeData);
  }
  addChildRow(): void {
    if (!this.selectedRow) {
    ...
    this.nextRowId = this.nextRowId - 1;
    const parentData = this.selectedRow.data || this.selectedRow;
    const parentId = parentData.ID;

    // Find parent node in tree
    const findAndAddChild = (nodes: TreeNode[], targetId: number): boolean => {
      for (const node of nodes) {
        if (Number(node.key) === targetId) {
          ...
          this.productTreeData = [...this.productTreeData];
          this.dataPOKHProduct = this.convertFromTreeNode(this.productTreeData);
          return true;
        }
        if (node.children && node.children.length > 0) {
          if (findAndAddChild(node.children, targetId)) {
            return true;
          }
        }
      }
    ...
    findAndAddChild(this.productTreeData, parentId);
  }
  ...
  //           },
  //           bottomCalc: (values, data) => {
  //             return this.accumulateTreeValues(data, 'DiscountAmount');
  //           },
  //           bottomCalcFormatter: 'money',
  //           bottomCalcFormatterParams: {
  //             precision: 0,
    ...
    modalRef.result.then(
      (result: any) => {
        if (result && result.success && result.processedData) {
          ...
          // Chuyển đổi dữ liệu thành cấu trúc tree với _children
          ...
          // Convert to PrimeNG TreeNode format
          this.dataPOKHProduct = treeData;
          this.productTreeData = this.convertToTreeNode(treeData);
    ...
    modalRef.result.then(
      (result: any) => {
        if (result && result.success && result.dtAddDetail) {
          ...
          const existingData = this.convertFromTreeNode(this.productTreeData);
          ...
          // Convert to PrimeNG TreeNode and update
          this.productTreeData = this.convertToTreeNode(mergedData);
          this.dataPOKHProduct = mergedData;
  ...
  copyPOKHToDTO() {
    ...
    // Flatten chi tiết sản phẩm từ PrimeNG TreeTable
    const detailsFlat = this.convertFromTreeNode(this.productTreeData);
