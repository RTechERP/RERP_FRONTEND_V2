...
@Component({
  selector: 'app-pokh-detail-slickgrid',
  imports: [
    NzCardModule,
    FormsModule,
    ReactiveFormsModule,
  ...
  styleUrl: './pokh-detail-slickgrid.component.css',
})
export class PokhDetailSlickgridComponent implements OnInit, AfterViewInit {
  ...
  tb_POKHDetailFile!: Tabulator; // OLD - Tabulator
  tb_ProductDetailTreeList!: Tabulator;
  tb_DetailUser!: Tabulator;

  // SlickGrid properties for POKHDetailFile table
  angularGridPOKHDetailFile!: AngularGridInstance;
  columnDefinitionsPOKHDetailFile: Column[] = [];
  gridOptionsPOKHDetailFile: GridOption = {};
  datasetPOKHDetailFile: any[] = [];

  // SlickGrid properties for POKHDetailProduct tree table
  angularGridPOKHDetailProduct!: AngularGridInstance;
  columnDefinitionsPOKHDetailProduct: Column[] = [];
  gridOptionsPOKHDetailProduct: GridOption = {};
  datasetPOKHDetailProduct: any[] = [];
  ...
  ngAfterViewInit(): void {
    if (this.isEditMode && this.selectedId > 0 && !this.isLoadingData) {
      ...
      this.initGridPOKHDetailProduct(); // Khởi tạo SlickGrid với dữ liệu rỗng
      this.datasetPOKHDetailProduct = [];

      this.initGridPOKHDetailFile(); // Khởi tạo SlickGrid với dữ liệu rỗng
      this.datasetPOKHDetailFile = [];
    ...
    this.POKHService.loadProducts().subscribe({
      next: (response: any) => {
        if (response.status === 1) {
          ...
          this.initGridPOKHDetailProduct();
          this.datasetPOKHDetailProduct = this.convertToFlatTreeData(this.dataPOKHProduct);
        } else {
          this.notification.error(
            'Thông báo',
        ...
        if (response.status === 1) {
          ...
          forkJoin([POKHProducts$, POKHFiles$, detailUser$]).subscribe(
            ([productsData, filesData, userDetailsData]) => {
              ...
              this.initGridPOKHDetailFile(); // Init SlickGrid for files
              this.datasetPOKHDetailFile = this.dataPOKHDetailFile.map((file, index) => ({
                ...file,
                id: file.id || file.ID || Date.now() + index, // id duy nhất cho SlickGrid
                STT: index + 1,
              }));
  ...
  savePOKH() {
    ...
    const details = this.datasetPOKHDetailProduct;
    const detailUsers =
  ...
  calculateTotalMoneyKoVAT() {
    ...
    processRows(this.dataPOKHProduct);
    return total;
  }
  ...
  private convertToFlatTreeData(treeData: any[]): any[] {
    const flatData: any[] = [];
    let stt = 1;

    const processNode = (node: any, parentId: number | null = null, level: number = 0) => {
      const flatNode = {
        ...node,
      ...
      if (node._children && node._children.length > 0) {
        node._children.forEach((child: any) => {
          processNode(child, flatNode.id, level + 1);
        });
      }
    };
  ...
  addChildRow(): void {
    ...
    const childRow = {
      ID: this.nextRowId,
      STT: parentData._children ? parentData._children.length + 1 : 1,
      ProductNewCode: '',
      ProductCode: '',
      ProductName: '',
      GuestCode: '',
    ...
    // Add child to parent in dataPOKHProduct
    const addToParent = (nodes: any[]): boolean => {
      for (const node of nodes) {
        if (node.ID === parentData.ID) {
          if (!node._children) node._children = [];
          node._children.push(childRow);
          return true;
        }
        if (node._children && node._children.length > 0) {
          if (addToParent(node._children)) return true;
        }
      }
      return false;
    };

    addToParent(this.dataPOKHProduct);
    this.datasetPOKHDetailProduct = this.convertToFlatTreeData(this.dataPOKHProduct);
  }
  ...
  addFileToTable(file: File): void {
    const newFile = {
      id: Date.now() + Math.random(), // id duy nhất cho SlickGrid
    ...
    // Cập nhật dataset cho SlickGrid với STT
    this.datasetPOKHDetailFile = this.dataPOKHDetailFile.map((file, index) => ({
      ...file,
      STT: index + 1,
    }));

    // Force grid refresh sau khi thêm data
    setTimeout(() => {
      if (this.angularGridPOKHDetailFile?.dataView) {
        this.angularGridPOKHDetailFile.dataView.setItems(this.datasetPOKHDetailFile);
        this.angularGridPOKHDetailFile.slickGrid?.invalidate();
        this.angularGridPOKHDetailFile.slickGrid?.render();
      }
      // Xóa element "No data to display" bằng DOM
      const container = document.querySelector('.grid-container-pokh-detail-file');
      if (container) {
        const emptyWarnings = container.querySelectorAll('[class*="empty"], [class*="warning"]');
        emptyWarnings.forEach((el: any) => {
          if (el.textContent?.includes('No data') || el.textContent?.includes('no data')) {
  ...
  //#endregion
  //#region : Các hàm vẽ bảng - SlickGrid for Product Detail Tree table
  initGridPOKHDetailProduct(): void {
    this.columnDefinitionsPOKHDetailProduct = [
      {
        id: 'actions',
        name: '',
        field: 'id',
        ...
        formatter: (row: number, cell: number, value: any, columnDef: any, dataContext: any) => {
          return `<button class="btn-delete-product text-danger p-0 border-0 bg-transparent" style="font-size: 0.8rem;" title="Xóa sản phẩm">
            <i class="fas fa-trash"></i>
          </button>`;
        },
        onCellClick: (e: any, args: any) => {
        ...
        formatter: (row: number, cell: number, value: any, columnDef: any, dataContext: any) => {
          ...
          return `
            <button class="btn-toggle-detail w-100 h-100" style="background-color: white; border: 1px solid #d9d9d9;" title="${productNewCode || 'Chọn sản phẩm'}">
              <span class="product-code-text">${productNewCode || 'Chọn SP'}</span>
              <span class="arrow">&#9662;</span>
            </button>
          `;
        },
        onCellClick: (e: any, args: any) => {
          this.showProductPopupForSlickGrid(args);
        },
      },
      { id: 'ProductCode', name: 'Mã Sản Phẩm (Cũ)', field: 'ProductCode', width: 120, minWidth: 100, sortable: true, editor: { model: Editors['text'] } },
      { id: 'ProductName', name: 'Tên sản phẩm', field: 'ProductName', width: 150, minWidth: 100, sortable: true, editor: { model: Editors['text'] } },
      { id: 'GuestCode', name: 'Mã theo khách', field: 'GuestCode', width: 150, minWidth: 100, sortable: true, editor: { model: Editors['text'] } },
      { id: 'Maker', name: 'Hãng', field: 'Maker', width: 100, minWidth: 80, sortable: true },
      { id: 'Qty', name: 'Số lượng', field: 'Qty', width: 80, minWidth: 60, sortable: true, editor: { model: Editors['integer'] }, formatter: this.moneyFormatter },
      { id: 'FilmSize', name: 'Kích thước phim cắt/Model', field: 'FilmSize', width: 200, minWidth: 100, sortable: true, editor: { model: Editors['text'] } },
      { id: 'Spec', name: 'Thông số kỹ thuật', field: 'Spec', width: 150, minWidth: 100, sortable: true, editor: { model: Editors['text'] } },
      { id: 'Unit', name: 'ĐVT', field: 'Unit', width: 100, minWidth: 60, sortable: true },
      { id: 'NetUnitPrice', name: 'Đơn giá NET', field: 'NetUnitPrice', width: 150, minWidth: 100, sortable: true, editor: { model: Editors['float'] }, formatter: this.moneyFormatter },
      { id: 'UnitPrice', name: 'Đơn giá trước VAT', field: 'UnitPrice', width: 150, minWidth: 100, sortable: true, editor: { model: Editors['float'] }, formatter: this.moneyFormatter },
      { id: 'IntoMoney', name: 'Tổng tiền trước VAT', field: 'IntoMoney', width: 150, minWidth: 100, sortable: true, editor: { model: Editors['float'] }, formatter: this.moneyFormatter },
      { id: 'VAT', name: 'VAT (%)', field: 'VAT', width: 100, minWidth: 60, sortable: true, editor: { model: Editors['float'] }, formatter: this.vatFormatter },
      { id: 'TotalPriceIncludeVAT', name: 'Tổng tiền sau VAT', field: 'TotalPriceIncludeVAT', width: 150, minWidth: 100, sortable: true, editor: { model: Editors['float'] }, formatter: this.moneyFormatter },
      { id: 'UserReceiver', name: 'Người nhận', field: 'UserReceiver', width: 130, minWidth: 100, sortable: true, editor: { model: Editors['text'] } },
      { id: 'DeliveryRequestedDate', name: 'Ngày yêu cầu giao hàng', field: 'DeliveryRequestedDate', width: 200, minWidth: 120, sortable: true, formatter: this.dateFormatter },
      { id: 'EstimatedPay', name: 'Thanh toán dự kiến', field: 'EstimatedPay', width: 150, minWidth: 100, sortable: true, editor: { model: Editors['float'] }, formatter: this.moneyFormatter },
      { id: 'BillDate', name: 'Ngày hóa đơn', field: 'BillDate', width: 150, minWidth: 100, sortable: true, formatter: this.dateFormatter },
      { id: 'BillNumber', name: 'Số hóa đơn', field: 'BillNumber', width: 150, minWidth: 100, sortable: true, editor: { model: Editors['text'] } },
      { id: 'Debt', name: 'Công nợ', field: 'Debt', width: 150, minWidth: 100, sortable: true, editor: { model: Editors['float'] }, formatter: this.moneyFormatter },
      { id: 'PayDate', name: 'Ngày yêu cầu thanh toán', field: 'PayDate', width: 150, minWidth: 100, sortable: true, formatter: this.dateFormatter },
      { id: 'ProductPO', name: 'Nhóm', field: 'ProductPO', width: 100, minWidth: 80, sortable: true, editor: { model: Editors['text'] } },
      { id: 'Note', name: 'Ghi chú', field: 'Note', width: 100, minWidth: 80, sortable: true, editor: { model: Editors['text'] } },
    ];

    this.gridOptionsPOKHDetailProduct = {
      enableAutoResize: true,
      autoResize: {
        container: '.grid-container-pokh-detail-product',
      ...
      contextMenu: {
        ...
        onCommand: (e, args) => this.handleProductContextCommand(e, args),
      },
    };
  }

  angularGridReadyPOKHDetailProduct(angularGrid: AngularGridInstance): void {
    this.angularGridPOKHDetailProduct = angularGrid;
  }

  onProductDetailCellChange(eventData: any): void {
    const item = eventData?.dataContext;
    const columnField = eventData?.columnDef?.field;
    if (item && columnField) {
      this.handleCellValueChangeForSlickGrid(item, columnField);
    }
  }

  onProductDetailRowClick(eventData: any): void {
  ...
  vatFormatter(row: number, cell: number, value: any, columnDef: any, dataContext: any): string {
  ...
  deleteProductRow(item: any): void {
    this.modal.confirm({
      ...
      nzOnOk: () => {
        try {
          const idsToDelete = this.getAllRowIdsFromData(item);
          this.deletedPOKHDetailIds.push(...idsToDelete);

          if (item.parentId) {
            this.datasetPOKHDetailProduct = this.datasetPOKHDetailProduct.map((row: any) => {
              if (row.id === item.parentId && row._children) {
                row._children = row._children.filter((child: any) => child.id !== item.id);
              }
              return row;
            });
          } else {
            this.datasetPOKHDetailProduct = this.datasetPOKHDetailProduct.filter((row: any) => row.id !== item.id);
          }

          this.dataPOKHProduct = this.datasetPOKHDetailProduct;
  ...
  getAllRowIdsFromData(item: any): number[] {
    const ids: number[] = [];
    if (item.ID) {
      ids.push(item.ID);
    }
    if (item._children && item._children.length > 0) {
      item._children.forEach((child: any) => {
        ids.push(...this.getAllRowIdsFromData(child));
      });
    }
    return ids;
  }

  handleProductContextCommand(e: any, args: any): void {
    const item = args?.dataContext;
    if (!item) return;
  ...
  showProductPopupForSlickGrid(args: any): void {
    const item = args?.dataContext;
    if (!item) return;

    const cellElement = args?.cellNode;
    ...
    this.tabulatorPopupService.open(
      {
        ...
        onRowSelected: (selectedProduct) => {
          ...
          item.NetUnitPrice = selectedProduct.NetUnitPrice || 0;
          item.UnitPrice = selectedProduct.NetUnitPrice || 0;

          this.handleCellValueChangeForSlickGrid(item, 'ProductID');
          this.tabulatorPopupService.close();
        },
        onClosed: () => {
        },
  ...
  handleCellValueChangeForSlickGrid(item: any, columnField: string): void {
    if (this.lockEvents) return;
    ...
    try {
      if (unitPrice >= 0 && quantity > 0) {
        if (
          ...
          item.TotalPriceIncludeVAT = totalWithVAT;
          this.calculateTotalIterativeForSlickGrid();
        }
      }
      ...
      if (columnField === 'BillDate' || columnField === 'Debt') {
        if (billDate) {
          ...
          const month = String(payDate.getMonth() + 1).padStart(2, '0');
          const day = String(payDate.getDate()).padStart(2, '0');
          item.PayDate = `${year}-${month}-${day}T00:00:00`;
        }
      }

      if (columnField === 'Qty' || columnField === 'UnitPrice') {
        item.IntoMoney = quantity * unitPrice;
        this.calculateTotalIterativeForSlickGrid();
      }
    } catch (error) {
      this.notification.error(NOTIFICATION_TITLE.error, 'Lỗi:' + error);
  ...
  calculateTotalIterativeForSlickGrid(): void {
    let totalSum = 0;
    const processNode = (node: any) => {
      if (
        node['TotalPriceIncludeVAT'] !== undefined &&
        !isNaN(node['TotalPriceIncludeVAT'])
      ) {
        totalSum += Number(node['TotalPriceIncludeVAT']);
      }
      if (node._children && node._children.length > 0) {
        node._children.forEach(processNode);
      }
    };

    this.datasetPOKHDetailProduct.forEach(processNode);
    this.poFormData.totalPO = isNaN(totalSum) ? 0 : totalSum;
    console.log('Tổng giá trị sau VAT:', this.poFormData.totalPO);
  }

  //#endregion
  //#region : Các hàm vẽ bảng - SlickGrid for File table
  initGridPOKHDetailFile(): void {
    this.columnDefinitionsPOKHDetailFile = [
      {
        id: 'actions',
        name: '',
        field: 'id',
        ...
        resizable: false,
        formatter: (row: number, cell: number, value: any, columnDef: any, dataContext: any) => {
          return `<button class="btn-delete-file text-danger p-0 border-0 bg-transparent" style="font-size: 0.8rem;" title="Xóa file">
            <i class="fas fa-trash"></i>
          </button>`;
        },
        onCellClick: (e: any, args: any) => {
            ...
            this.modal.confirm({
              ...
              nzOnOk: () => {
                try {
                  ...
                  this.datasetPOKHDetailFile = this.datasetPOKHDetailFile.filter(
                    (row: any) => row.id !== item.id
                  );
                  this.dataPOKHDetailFile = this.dataPOKHDetailFile.filter(
                    (row: any) => row.id !== item.id
                  );
                } catch (error) {
      ...
      { id: 'STT', name: 'STT', field: 'STT', width: 50, minWidth: 50, sortable: true },
      { id: 'fileName', name: 'Tên file', field: 'fileName', width: 150, minWidth: 100, sortable: true, filterable: true },
      { id: 'fileType', name: 'Loại file', field: 'fileType', width: 70, minWidth: 50, sortable: true },
      { id: 'CreatedDate', name: 'Ngày tải lên', field: 'CreatedDate', width: 100, minWidth: 80, sortable: true },
    ];

    this.gridOptionsPOKHDetailFile = {
      enableAutoResize: true,
      autoResize: {
        container: '.grid-container-pokh-detail-file',
        calculateAvailableSizeBy: 'container',
        resizeDetection: 'container',
      },
      enableCellNavigation: true,
  ...
  angularGridReadyPOKHDetailFile(angularGrid: AngularGridInstance): void {
    this.angularGridPOKHDetailFile = angularGrid;
  }

  onPOKHDetailFileRowDblClick(e: any, args: any): void {
    const item = args?.dataContext;
    if (item && item['ID']) {
  ...
  private handleContextFileMenuCommand(e: any, args: MenuCommandItemCallbackArgs): void {
    const item = args?.dataContext;
    if (!item) return;
  ...
  */
  ...
  // OLD - Tabulator implementation (migrated to SlickGrid)
  /*
