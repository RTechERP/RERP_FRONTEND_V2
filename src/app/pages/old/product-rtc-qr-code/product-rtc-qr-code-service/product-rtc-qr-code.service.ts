...
export class ProductRtcQrCodeService {
  ...
  getQRCodeList(warehouseID: number, filterText: string = ''): Observable<any> {
    const params = new HttpParams()
      .set('wearHouseID', warehouseID.toString())
      .set('filterText', filterText);
    return this.http.get<any>(`${this.url}get-product-and-qrcode`, { params });
  }
  ...
  getProductRTC(warehouseID: number, serialNumber: string = ''): Observable<any> {
    return this.http.post<any>(`${this.url}get-productRTC`, { WarehouseID: warehouseID, serialNumber: serialNumber });
  }
