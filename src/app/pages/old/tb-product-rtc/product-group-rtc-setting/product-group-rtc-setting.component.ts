...
import { ProductsaleServiceService } from '../../Sale/ProductSale/product-sale-service/product-sale-service.service';
import { TbProductGroupRtcFormComponent } from '../tb-product-group-rtc-form/tb-product-group-rtc-form.component';

interface ProductGroupRTC {
  ID?: number;
...
@Component({
  ...
  templateUrl: './product-group-rtc-setting.component.html',
  styleUrl: './product-group-rtc-setting.component.css'
})
export class ProductGroupRtcSettingComponent implements OnInit, AfterViewInit {
  constructor(
  ...
  newProductGroupRTC: ProductGroupRTC = {
    ProductGroupID: '',
    ProductGroupName: '',
    EmployeeID: 0,
    IsVisible: false,
  ...
  initGridOptions() {
    this.gridOptionsMaster = {
      ...
      autoResize: {
        container: '.grid-container-product-group-rtc',
  ...
  onVisibleProductGroup(isVisible: boolean) {
    ...
    this.modal.confirm({
      ...
      nzOnOk: () => {
        ...
        this.isLoading = true;
        this.tbProductRtcService.visibleProductGroup(updateData).subscribe({
          next: (_res: any) => {
