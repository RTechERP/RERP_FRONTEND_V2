...
export class PokhHistoryComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.loadCustomers();
    this.loadPOKHHistory(this.filters.filterText, this.filters.startDate, this.filters.endDate, this.filters.customerId)
  }
  ...
  loadPOKHHistory(keywords: string, startDate: Date, endDate: Date, cusCode: string): void {
    this.POKHHistoryService.loadData(keywords, startDate, endDate, cusCode).subscribe(
      response => {
        if (response.status === 1) {
          ...
          this.notification.error('Lỗi khi tải lịch sử POKH:', response.message);
        }
      },
      error => {
  ...
  async exportDetailTableToExcel() {
    ...
    currentPageData.forEach((rowData, rowIndex) => {
      const row = worksheet.addRow(columns.map(col => {
        ...
        // Format date columns
        if (columnFormats[field] && columnFormats[field].includes('dd/mm/yyyy')) {
          if (value && value !== '') {
    ...
    link.href = url;
    link.download = `POKHHistory_List_Page_${currentPage}_${new Date().toISOString().split('T')[0]}.xlsx`;
    link.click();
  ...
  initTable(): void {
    this.tb_Table = new Tabulator(`#tb_Table`, {
      ...
      resizableRows: true,
      columns: [
        { title: 'Mã khách', field: 'CustomerCode', sorter: 'string', width: 150 },
        ...
        {
          title: 'Ngày PO', field: 'PODate', sorter: 'string', width: 150, formatter: (cell) => {
            const value = cell.getValue();
            return value ? DateTime.fromISO(value).toFormat('dd/MM/yyyy') : '';
          }
        },
        { title: 'Mã hàng', field: 'ProductCode', sorter: 'string', width: 150 },
        { title: 'Model', field: 'Model', sorter: 'string', width: 150 },
        { title: 'SL', field: 'Quantity', sorter: 'string', width: 150 },
        { title: 'SL giao', field: 'QuantityDeliver', sorter: 'string', width: 150 },
        { title: 'SL pending', field: 'QuantityPending', sorter: 'string', width: 150 },
        { title: 'ĐVT', field: 'Unit', sorter: 'string', width: 150 },
        {
          title: 'Giá net', field: 'NetPrice', sorter: 'string', width: 150, formatter: "money",
          formatterParams: {
            precision: 0,
            decimal: ".",
            thousand: ",",
        ...
        {
          title: 'Đơn giá', field: 'UnitPrice', sorter: 'string', width: 150, formatter: "money",
          formatterParams: {
            precision: 0,
            decimal: ".",
            thousand: ",",
        ...
        {
          title: 'Thành tiền', field: 'TotalPrice', sorter: 'string', width: 150, formatter: "money",
          formatterParams: {
            precision: 0,
            decimal: ".",
            thousand: ",",
        ...
        {
          title: 'VAT', field: 'VAT', sorter: 'string', width: 150, formatter: "money",
          formatterParams: {
            precision: 0,
            decimal: ".",
            thousand: ",",
        ...
        {
          title: 'Tổng tiền sau VAT', field: 'TotalPriceVAT', sorter: 'string', width: 150, formatter: "money",
          formatterParams: {
            precision: 0,
            decimal: ".",
            thousand: ",",
        ...
        {
          title: 'Giao hàng thực tế', field: 'DeliverDate', sorter: 'string', width: 150,
          formatter: (cell) => {
            const value = cell.getValue();
            return value ? DateTime.fromISO(value).toFormat('dd/MM/yyyy') : '';
          }
        ...
        {
          title: 'Thanh toán thực tế', field: 'PaymentDate', sorter: 'string', width: 150, formatter: (cell) => {
            const value = cell.getValue();
            return value ? DateTime.fromISO(value).toFormat('dd/MM/yyyy') : '';
          }
        },
        {
          title: 'Ngày hóa đơn', field: 'BillDate', sorter: 'string', width: 150, formatter: (cell) => {
            const value = cell.getValue();
            return value ? DateTime.fromISO(value).toFormat('dd/MM/yyyy') : '';
          }
        },
        { title: 'Số hóa đơn', field: 'BillNumber', sorter: 'string', width: 150 },
