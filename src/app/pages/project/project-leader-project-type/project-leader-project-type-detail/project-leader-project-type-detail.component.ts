...
export class ProjectLeaderProjectTypeDetailComponent implements OnInit {
  ...
  ngAfterViewInit() {
    ...
    this.drawTbProjectEmployeeLinks(
      this.tb_projectEmployeeLinkContainer.nativeElement
    );
    this.getProjectEmployeefilter();
  }
        ...
        next: (response: any) => {
          ...
          this.tb_projectEmployeeLinks.setData(data).then(() => {
            ...
            if (idsToReselect.length) {
              ...
              setTimeout(
                () => this.tb_projectEmployeeLinks.selectRow(idsToReselect),
                0
              );
            }
          });
          ...
          .subscribe({
            next: (response: any) => {
              ...
              if (duplicateEmployeeIDs.length > 0) {
                const currentRows =
                  this.tb_projectEmployeeLinks?.getData?.() || [];
                const idToCode = new Map(
                  currentRows.map((r: any) => [r.ID, r.Code])
                );
                const duplicateCodes = duplicateEmployeeIDs.map(
                  (id) => idToCode.get(id) || `ID:${id}`
                );
                this.notification.warning(
                  'Thông báo',
              ...
              this.projectService.saveemployeeprojecttype(payload).subscribe({
                next: (res) => {
                  this.notification.success(
                    'Thành công',
                    `Đã thêm ${payload.length} nhân viên thành công!`
                  );
