...
export class PONCCService {
  ...
  getAll(filter: PONCCFilter): Observable<any> {
    ...
    if (filter.SupplierID)
      httpParams = httpParams.set('supplierID', filter.SupplierID.toString());
    if (filter.EmployeeID)
      httpParams = httpParams.set('employeeID', filter.EmployeeID.toString());
    ...
    if (filter.POType)
      httpParams = httpParams.set('poType', filter.POType.toString());

    return this.http
      .get<any>(`${this.baseUrl}get-all`, { params: httpParams })
      .pipe(
        map((res: any) => ({
  ...
  validateRequestImport(ids: number[], warehouseID: number): Observable<any> {
    return this.http.post<any>(
      `${this.baseUrl}validate-request-import?warehouseID=${warehouseID}`,
      ids
    );
  }
  ...
  getPonccDetail(
    ...
    return this.http.get<any>(
      `${this.baseUrl}poncc-detail?idText=${idText}&warehouseID=${warehouseID}&detailId=${detailId}`
    );
  }

  getWarehouseCode(warehouseID: number): Observable<any> {
    return this.http.get<any>(
      `${this.baseUrl}warehouse-code?warehouseId=${warehouseID}`
    );
  }

  getBillImportTech(billimportTechId: number): Observable<any> {
    return this.http.get<any>(
      `${this.baseUrl}bill-import-tech?billimportTechId=${billimportTechId}`
    );
  }

  getReceivedId(warehouseID: number, productGroupID: number): Observable<any> {
    return this.http.get<any>(
      `${this.baseUrl}receivedId?warehouseID=${warehouseID}&productGroupID=${productGroupID}`
    );
  }
  ...
  getSuppliers(): Observable<any[]> {
    return this.http
      .get<any>(environment.host + 'api/suppliersale')
      .pipe(
        map((res: any) =>
  ...
  getEmployees(status: number = 0): Observable<any[]> {
    ...
    return this.http
      .get<any>(environment.host + 'api/Employee/get-employees', { params })
      .pipe(
        map((res: any) =>
          Array.isArray(res?.data) ? res.data : res?.data || res || []
        )
  ...
  getCurrencies(): Observable<any[]> {
    return this.http
      .get<any>(environment.host + 'api/Currency')
      .pipe(
        map((res: any) =>
          Array.isArray(res?.data) ? res.data : res?.data || res || []
        )
  ...
  getRulePayments(): Observable<any[]> {
    return this.http
      .get<any>(environment.host + 'api/RulePay')
      .pipe(
        map((res: any) =>
          Array.isArray(res?.data) ? res.data : res?.data || res || []
        )
  ...
  getProductSale(): Observable<any[]> {
    return this.http
      .get<any>(this.baseUrl + 'product-sale')
      .pipe(
        map((res: any) =>
          Array.isArray(res?.data) ? res.data : res?.data || res || []
        )
  ...
  getProductRTC(): Observable<any[]> {
    return this.http
      .get<any>(this.baseUrl + 'product-rtc')
      .pipe(
        map((res: any) =>
          Array.isArray(res?.data) ? res.data : res?.data || res || []
        )
  ...
  generatePOCode(supplierID: number, currencyID: number): Observable<any> {
    ...
    return this.http
      .get<any>(`${this.baseUrl}generate-po-code`, { params })
  ...
  generateBillCode(poType: number): Observable<any> {
    ...
    return this.http
      .get<any>(`${this.baseUrl}generate-bill-code`, { params })
      .pipe(map((res: any) => res?.data || ''));
  }

  getPOCode(productCode: string) {
    return this.http.get<any>(
      `${this.baseUrl}po-code?productCode=${productCode}`
    );
  }

  deletedPonccDetail(poDetailId: number) {
    return this.http.delete<any>(
      `${this.baseUrl}deleted-poncc-detail?poDetailId=${poDetailId}`
    );
  }

  getBillCode(poTypeId: number) {
    return this.http.get<any>(
      `${this.baseUrl}load-bill-code?poTypeId=${poTypeId}`
    );
  }

  getHistoryPrice(productId: number, productCode: string) {
    return this.http.get<any>(
      `${this.baseUrl}history-price?productId=${productId}&productCode=${productCode}`
    );
  }

  checkPoCode(id: number, pOCode: string, billCode: string) {
    return this.http.get<any>(
      `${this.baseUrl}check-po-code?id=${id}&pOCode=${pOCode}&billCode=${billCode}`
    );
  }
  ...
  getPONCCSummary(request: {
    ...
    return this.http
      .post<any>(`${this.baseUrl}get-po-ncc-summary`, request)
      .pipe(
        map((res: any) => ({
