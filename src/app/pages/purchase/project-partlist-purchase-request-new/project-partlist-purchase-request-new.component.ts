...
import { RequestType, Currency } from '../project-partlist-purchase-request/project-partlist-purchase-request.model';
...
import { ProjectPartlistPurchaseRequestDetailComponent } from '../project-partlist-purchase-request/project-partlist-purchase-request-detail/project-partlist-purchase-request-detail.component';
...
import { HistoryPriceComponent } from '../project-partlist-purchase-request/history-price/history-price.component';
...
@Component({
  selector: 'app-project-partlist-purchase-request-new',
  imports: [
    CommonModule,
    FormsModule,
    NzButtonModule,
...
export class ProjectPartlistPurchaseRequestNewComponent implements OnInit, AfterViewInit {
...
private validateManufacturerForVision(rows: any[]): boolean {
  ...
  ngAfterViewInit(): void {
    ...
    // Không gọi initTables() ngay ở đây vì tabs chưa có dữ liệu
    // Sẽ được gọi sau khi getRequestTypes() hoàn thành
  }
  //#endregion
  ...
  getRequestTypes() {
    this.srv.getRequestTypes().subscribe({
      next: (types: RequestType[]) => {
        ...
        // Sau khi tabs được tạo, khởi tạo bảng
        setTimeout(() => {
          this.initTables();
        }, 100);
        this.onSearch();
  ...
  private initTables() {
    if (!this.containers || this.containers.length === 0) {
      // Nếu containers chưa sẵn sàng, thử lại sau một chút
      setTimeout(() => {
        if (this.containers && this.containers.length > 0) {
          this.initTables();
        }
    ...
    // Nếu đã có tables, destroy chúng trước
  ...
  //#region PHASE 1 – COMMON UTIL (BẮT BUỘC)
  // Formatter dùng lại
  ...
  //#region PHASE 2 – BASE COLUMN CONFIG (QUAN TRỌNG)
  ...
  //#region PHASE 3 – SPECIAL LOGIC MAP (THAY SWITCH)
  // Checkbox fields (readonly)
  ...
  //#region PHASE 5 – NHỮNG FIELD CỰC KỲ ĐẶC BIỆT (GIỮ RIÊNG)
  private applySpecialFieldLogic(field: string, colDef: any, tabIndex: number): void {
    switch (field) {
      ...
      case 'ProductCodeRTC':
        // Cột mã nội bộ cho tab mượn demo
        colDef.width = this.COLUMN_WIDTH_CONFIG['ProductNewCode'] || 100;
        colDef.hozAlign = 'left';
      ...
      case 'ProductGroupRTCID':
        // Cột loại kho cho tab mượn demo
        ...
        colDef.editorParams = (cell: any) => {
          const rowData = cell.getRow().getData();
          // Ở tab mượn demo, luôn dùng productRTC
          return {
      ...
      case 'BillCode':
      case 'POKHCode':
      case 'ParentProductCodePO':
      case 'GuestCode':
      case 'StatusPOKHText':
      case 'SpecialCode':
      case 'FullNamePriceRequest':
  ...
  private GetTableConfig(tabIndex: number = 0): any {
    // Build columns từ FORM_COLUMN_ORDER
    const columns = this.FORM_COLUMN_ORDER.map(field => {
      // Ở tab mua demo (tabIndex = 2) và mượn demo (tabIndex = 3), thay ProductNewCode bằng ProductCodeRTC và ProductGroupID bằng ProductGroupRTCID
    ...
    // Trả về config object với columns
    return {
  ...
  async updateInternalCode(idx: number) {
    ...
    this.modal.confirm({
      ...
      nzOnOk: () => {
        ...
        this.srv.createProductRTC(dataToUpdate).subscribe({
          next: (rs) => {
            // Lấy dữ liệu trả về từ API
            const responseData = rs?.data || rs || [];
            const dataArray = Array.isArray(responseData) ? responseData : [responseData];
            
            // Tạo map để tìm nhanh dữ liệu theo ID
            ...
            // Cập nhật các dòng trong bảng với ProductRTCID và ProductCodeRTC
            const table = this.tables[idx];
            if (table && dataMap.size > 0) {
              const allRows = table.getRows();
              let hasUpdates = false;
              
              allRows.forEach((row: any) => {
                const rowData = row.getData();
                const rowId = Number(rowData['ID'] || 0);
                
                if (rowId > 0 && dataMap.has(rowId)) {
                  const updatedData = dataMap.get(rowId);
                  
                  // Cập nhật ProductRTCID và ProductCodeRTC
                  const updateFields: any = {};
                  if (updatedData['ProductRTCID'] !== undefined) {
                    updateFields['ProductRTCID'] = updatedData['ProductRTCID'];
                  }
                  if (updatedData['ProductCodeRTC'] !== undefined) {
                    updateFields['ProductCodeRTC'] = updatedData['ProductCodeRTC'];
                    // Cập nhật cả ProductNewCode để tương thích với các tab khác
                    // updateFields['ProductNewCode'] = updatedData['ProductCodeRTC'];
                  }
                  
                  if (Object.keys(updateFields).length > 0) {
                    hasUpdates = true;
                    
                    // Cập nhật trong allData trước
                    const allDataIndex = this.allData.findIndex((item: any) => Number(item['ID']) === rowId);
                    if (allDataIndex !== -1) {
                      Object.assign(this.allData[allDataIndex], updateFields);
                    }
                    
                    // Cập nhật trong dataByType
                    this.dataByType.forEach((dataArray, typeId) => {
                      const typeDataIndex = dataArray.findIndex((item: any) => Number(item['ID']) === rowId);
                      if (typeDataIndex !== -1) {
                        Object.assign(dataArray[typeDataIndex], updateFields);
                      }
                    });
                    
                    // Cập nhật lại originalDataMap để đồng bộ
                    const originalData = this.originalDataMap.get(rowId);
                    if (originalData) {
                      Object.assign(originalData, updateFields);
                      this.originalDataMap.set(rowId, originalData);
                    }
                    
                    // Cập nhật row trong table
                    row.update(updateFields);
                  }
                }
              });
              
              // Force redraw toàn bộ table sau khi update tất cả các row
              if (hasUpdates) {
                setTimeout(() => {
                  try {
                    table.redraw(true);
            ...
            this.isLoading = false;
            // Không cần reload toàn bộ vì đã update trực tiếp vào row và data
          },
          error: (error) => {
            this.notify.error(NOTIFICATION_TITLE.error, error.error.message);
            this.isLoading = false;
            ...
            nzOnOk: () => {
              ...
              this.suplierSaleService.getSupplierSaleByID(listSupplierSale[0]).subscribe({
                next: (rs) => {
                  ...
                  const isMuonDemoTab = this.activeTabIndex === 3;
                  ...
                  console.log('=== DEBUG CREATE PO NCC ===');
                  console.log('activeTabIndex:', this.activeTabIndex);
                  console.log('isMuonDemoTab:', isMuonDemoTab);
                  console.log('hasMuonProduct:', hasMuonProduct);
                  console.log('poType:', poType);
                  
                  // Lấy số đơn hàng (BillCode) theo POType
                  this.ponccService.getBillCode(poType).subscribe({
                    next: (billCodeRes: any) => {
                      console.log('getBillCode response:', billCodeRes);
                      console.log('BillCode from API:', billCodeRes.data);
                      ...
                      console.log('poncc object before open modal:', poncc);
...
//           colDef.formatter = (cell: any) => this.formatNumberEnUS(cell.getValue(), 2);
//           colDef.bottomCalc = 'sum';
//           colDef.bottomCalcFormatter = (cell: any) => this.formatNumberEnUS(cell.getValue(), 2);
//           colDef.mutator = (value: any) => {
//             const v = Number(value);
//             return isNaN(v) ? 0 : v;
//           };
//           break;
