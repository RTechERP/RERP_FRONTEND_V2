...
export class TbProductRtcComponent implements OnInit, AfterViewInit {
  ...
  onEditGroup() {
    const selectedGroup = this.productGroupData.find(group => group.ID === this.productGroupID);
    if (!selectedGroup) {
      this.notification.warning('Thông báo', 'Vui lòng chọn một nhóm sản phẩm để sửa.');
      return;
    }
    const modalRef = this.ngbModal.open(TbProductGroupRtcFormComponent, {
      size: 'md',
  ...
  onDeleteProductGroup() {
    if (this.productData.length != 0) {
      this.notification.warning('Thông báo', 'Không thể xóa nhóm vì vẫn còn sản phẩm thuộc nhóm này.');
      return;
    }
    const selectedGroup = this.productGroupData.find(group => group.ID === this.productGroupID);
    if (!selectedGroup) {
  ...
  onDeleteProduct() {
    ...
    this.tbProductRtcService.saveData(payload).subscribe({
      next: () => {
        this.notification.success('Thành công', 'Xóa thiết bịws thành công!');
        this.getProduct();
        this.drawTable();
      },
      error: (err) => {
