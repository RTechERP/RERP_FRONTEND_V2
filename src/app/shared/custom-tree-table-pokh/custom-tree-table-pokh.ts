...
@Component({
    selector: 'app-custom-tree-table-pokh',
    standalone: true,
    imports: [
        ...
        CheckboxModule, TabsModule, NzDatePickerModule
    ],
    templateUrl: './custom-tree-table-pokh.html',
    styleUrl: './custom-tree-table-pokh.css',
    providers: [DatePipe]
})
export class CustomTreeTablePokh implements OnChanges, OnInit {
    ...
    // Convert Date to ISO string for API (e.g. "2026-06-08T00:00:00")
    toISODateString(date: Date | string | null): string | null {
        if (!date) return null;
        if (typeof date === 'string') {
            const d = new Date(date);
            if (isNaN(d.getTime())) return null;
            return this.datePipe.transform(d, 'yyyy-MM-ddTHH:mm:ss') || null;
        }
        return this.datePipe.transform(date, 'yyyy-MM-ddTHH:mm:ss') || null;
    }

    // Cache cho date parsing (tránh lag khi render)
    private dateCache: Map<string, Date | null> = new Map();

    // Convert ISO string to Date for nz-date-picker (with caching)
    ...
    // Get value for HTML date input (format: yyyy-MM-dd)
    getDateInputValue(dateStr: string | Date | null): string {
        if (!dateStr) return '';
    ...
    // Set value from HTML date input
    setDateInputValue(rowData: any, field: string, dateValue: string) {
        if (!dateValue) {
            rowData[field] = null;
        } else {
            rowData[field] = `${dateValue}T00:00:00`;
        }
    ...
    emitCellValueChange(rowData: any, field: string, value: any, forceEmit = false) {
        const processedValue = value instanceof Date ? this.toISODateString(value) : value;
        rowData[field] = processedValue;
        if (forceEmit) {
            this.cellValueChange.emit({ rowKey: rowData[this.dataKey] ?? rowData['_id'], rowData, field, value: processedValue });
        }
    }

    onActionsClick(event: Event, rowData: any) {
        event.stopPropagation();
        this.cellAction.emit({ field: 'actions', rowData });
    }

    onCellEditComplete(rowData: any, field: string) {
        const value = rowData[field];
        const processedValue = value instanceof Date ? this.toISODateString(value) : value;
        console.log('[TreeTable onCellEditComplete]', { field, value: processedValue, rowData: { STT: rowData.STT, ID: rowData.ID, id: rowData.id } });
        this.cellValueChange.emit({ rowKey: rowData[this.dataKey] ?? rowData['_id'], rowData, field, value: processedValue });
    }
    ...
    formatValue(col: TreeColumnDef, rowData: any): string {
        ...
        if (col.editType === 'date' && val) {
            const dateVal = this.toDate(val);
            if (dateVal) {
                return this.formatDate(dateVal, col.editDateFormat || 'dd/MM/yyyy', col.editShowTime);
            }
        }
